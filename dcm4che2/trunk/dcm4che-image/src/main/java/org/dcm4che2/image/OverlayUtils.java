...
public class OverlayUtils {
	...
	/** Returns true if the given frame number references an overlay - that is, is the form 0x60xx yyyy where
	 * xx is the overlay number, and yyyy is the overlay frame number.   xx must be even.
	 * @param imageIndex
	...
	public static int extractFrameNumber(int imageIndex) {
		if( isOverlay(imageIndex) ) return imageIndex & 0xFFFF;
	 ...
	 * This will return a bitmap overlay, in the colour specified.  The image reader isn't required unless overlays encoded
	 * in the high bits are being read.
	 * 
	 * @param ds is a DicomObject to read the overlay from.
	 * @param overlayNumber - of the form 0x60xx yyyy where xx is the overlay index and yyyy is the frame index
	 * @param reader is the image reader used to extract the raster for the high bits
	 * @param rgb is the colour to apply, can be null to use black & white overlays.
	 * @return A single bit buffered image, transparent except in the given colour where bits are 1.
	 * @throws IOException only if an image from the image reader is attempted and throws an exception.
	 */
	public static BufferedImage extractOverlay(DicomObject ds, int overlayNumber, ImageReader reader, String rgbs)
		...
		if (position == 0) {
			...
			// Extract a sub-frame IF there is a sub-frame, and one is specified.  There must be at least 2 frames worth
		...
		WritableRaster wr = Raster.createPackedRaster(db,
				cols,rows, 1, new Point());
		byte[] rArr = rgbArr;
		byte[] gArr = rgbArr;
		byte[] bArr = rgbArr;

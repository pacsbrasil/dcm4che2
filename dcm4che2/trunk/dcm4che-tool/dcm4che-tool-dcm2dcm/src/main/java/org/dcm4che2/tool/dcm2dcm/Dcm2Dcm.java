...
public class Dcm2Dcm {
	...
	public void convert(File src, File dest) throws IOException {
		...
		try {
			...
			if( fmiAttrs!=null ) sourceSyntaxUid = fmiAttrs.getString(Tag.TransferSyntaxUID,
	 ...
	 * Modifies meta to have a new dicom object if it is changing bit depth, 
	 * and updates various headers.
	 */
	protected LookupTable prepareBitStrip(DicomStreamMetaData meta, ImageReader reader) throws IOException {
		if(! destinationSyntax.uid().equals(UID.JPEGExtended24)) return null;
		DicomObject ds = meta.getDicomObject();
		int stored = ds.getInt(Tag.BitsStored);
		if( stored<13 ) return null;
	...
	/** Recodes the images from the source transfer syntax, as read from the src file, to the specified destination syntax. */
	public void recodeImages(File src, File dest) throws IOException {
		...
		for (int i = 0; i < frames; i++) {
			...
			if( lut!=null ) {
				lut.lookup(bi.getRaster().getDataBuffer(), bi.getRaster().getDataBuffer());
			}

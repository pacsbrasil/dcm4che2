...
public class SizeSkipInputHandler implements DicomInputHandler {
	...
	private long privateSkipSize = 1*1024*1024;
	private long publicSkipSize = Integer.MAX_VALUE;
	DicomInputHandler subHandler;

	public SizeSkipInputHandler(DicomInputHandler subHandler) {
	...
	/**
	 * Return true if this tag should be skipped/replaced with an alternate tag.
	 * @param in
	 * @return
	 */
	public boolean isSkip(DicomInputStream in) {
		if( TagUtils.isPrivateDataElement(in.tag() )) {
			return in.valueLength() >= privateSkipSize;
		}
		return in.valueLength() >= publicSkipSize;
	}

	public long getPrivateSkipSize() {
		return privateSkipSize;
	}

	public void setPrivateSkipSize(long privateSkipSize) {
		this.privateSkipSize = privateSkipSize;
	}

	public long getPublicSkipSize() {
		return publicSkipSize;
	}

	public void setPublicSkipSize(long publicSkipSize) {
		this.publicSkipSize = publicSkipSize;
	}

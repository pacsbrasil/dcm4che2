 ...
 * @since Mar 24, 2007
 */
public class NMFrameSelector extends AbstractHPSelector {

    private final DicomObject filterOp;
    private final Category cat;
    private final int[] selvals;
        ...
        public static Category valueOf(String name) {
            try {
                ...
                throw new IllegalArgumentException(
                        "(0072,0402) Filter-by Category: " + name);
            }
        }
    }
    
    public NMFrameSelector(DicomObject filterOp) {
        String catStr = filterOp.getString(Tag.FilterbyCategory);
        if (catStr == null) {
            throw new IllegalArgumentException(
                    "Missing (0072,0402) Filter-by Category");                            
        }
        ...
        int[] values = filterOp.getInts(Tag.SelectorUSValue);
        if (values == null || values.length == 0)
            throw new IllegalArgumentException(
                    "Missing (0072,007A) AbstractHPSelector US Value");
        this.selvals = values;
        this.filterOp = filterOp;
    }

    public NMFrameSelector(Category cat, int[] selvals) {
        this.cat = cat;
        this.selvals = (int[]) selvals.clone();
        this.filterOp = new BasicDicomObject();
        filterOp.putString(Tag.FilterbyCategory, VR.CS, cat.name);
        filterOp.putString(Tag.SelectorAttributeVR, VR.CS, "US");
        filterOp.putInts(Tag.SelectorUSValue, VR.US, selvals);
    }
    ...
    public boolean matches(DicomObject dcmobj, int frame) {
        int[] vals = dcmobj.getInts(cat.tag);
        if (vals == null || frame >= vals.length) {
            return true;
        }
        for (int i = 0; i < selvals.length; i++) {
            if (selvals[i] == vals[frame]) {
                return true;
            }
        }
        return false;

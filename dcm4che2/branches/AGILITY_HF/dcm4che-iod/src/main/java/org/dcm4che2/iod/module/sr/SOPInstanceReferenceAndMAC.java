...
public class SOPInstanceReferenceAndMAC extends SOPInstanceReferenceAndPurpose {
    ...
    public SOPInstanceReferenceAndMAC(DicomObject dcmobj) {
        super(dcmobj);
    }
    ...
    public static SOPInstanceReferenceAndMAC[] toSOPInstanceReferenceAndMACs(
            DicomElement sq) {
        if (sq == null || !sq.hasItems()) {
            return null;
        }
        SOPInstanceReferenceAndMAC[] a = new SOPInstanceReferenceAndMAC[sq
                .countItems()];
        for (int i = 0; i < a.length; i++) {
            a[i] = new SOPInstanceReferenceAndMAC(sq.getDicomObject(i));
        }
        return a;
    }

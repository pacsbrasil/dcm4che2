...
public class DcmMoverCli {
    ...
    private static final String DESCRIPTION = 
    	"Move a study from the specified Query/Retreive SCP to the specified\n"
    ...
    private static CommandLine parse(String[] args) {
        ...
        OptionBuilder.hasArg();
        OptionBuilder.withDescription(
        		"(QR SCU, STORE SCU) Timeout in ms for TCP connect, no timeout by default.");
        ...
        opts.addOption("tcpdelay", false,
        		"(All) Set TCP_NODELAY socket option to false, true by default.");

        OptionBuilder.withArgName("ms");
        OptionBuilder.hasArg();
        OptionBuilder.withDescription(
                "(QR SCU, STORE SCU) Timeout in ms for receiving A-ASSOCIATE-AC, 5s by default.");
        opts.addOption(OptionBuilder.create("acceptTO"));

        OptionBuilder.withArgName("ms");
        OptionBuilder.hasArg();
        OptionBuilder.withDescription(
                "(QR SCU, STORE SCU) Timeout in ms for receiving A-RELEASE-RP, 5s by default.");
        ...
        OptionBuilder.withDescription(
        		"Specify DICOM object attributes that should be removed during the" +
	...
	public static void main(String[] args) {
        ...
        if (cl.hasOption("rspTO"))
            dcmMover.setDimseRspTimeout(parseInt(cl.getOptionValue("rspTO"),
                    "illegal argument of option -rspTO", 1, Integer.MAX_VALUE));

        dcmMover.setTcpNoDelay(!cl.hasOption("tcpdelay"));

        if (cl.hasOption("acceptTO"))
        	dcmMover.setAcceptTimeout(parseInt(cl.getOptionValue("acceptTO"),
	...
	private static DcmMover createDcmMover(CommandLine cl) {
        if (cl.hasOption("dbi")) {
        	// Using a database to record study move progress
            ...
            if (cl.hasOption("dbPort")) {
            	port = new Integer(parseInt(cl.getOptionValue("dbPort"),
                        "illegal argument of option -connectTO", 1,
                        Integer.MAX_VALUE));
            }
        	String db = null;
	...
	private static void doMove(DcmMover mover, String studyUid, ObjectTransformData psd) {
		if (mover instanceof DcmMoverDbi) {
			System.out.print("\nDoing move using the database interface. Results will be written to the specified database.");

			DcmMoverDbi moverDbi = DcmMoverDbi.class.cast(mover);
			
			// This is an async move so just start it
			int moveId = moverDbi.startMove(studyUid, psd);
			
			System.out.print("\nMove started - check the database for progress. Move id = " + new Integer(moveId).toString());
		} else {
	...
	private static boolean addPatientStudyDataToXform(ObjectTransformData psd, int tag, String value) {
		switch (tag) {
		//
		// Patient attributes
		//

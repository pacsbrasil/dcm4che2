...
abstract class ContentImpl implements org.dcm4che.srom.Content {
    ...
    static ContentImpl newContent(KeyObject owner, Dataset ds, Date obsDateTime,
            Template template, Code name, String type)
            throws DcmValueException {
        ...
        if ("TEXT".equals(type)) {
            return new TextContentImpl(owner, obsDateTime, template, name,
        ...
        if ("DATE".equals(type)) {
            return new DateContentImpl(owner, obsDateTime, template, name,
                    ds.getDate(Tags.Date));
        }
        if ("TIME".equals(type)) {
            return new TimeContentImpl(owner, obsDateTime, template, name,
                    ds.getDate(Tags.Time));
        }
        if ("UIDREF".equals(type)) {
            return new UIDRefContentImpl(owner, obsDateTime, template, name,
                    ds.getString(Tags.UID));
        }
        if ("PNAME".equals(type)) {
            return new PNameContentImpl(owner, obsDateTime, template, name,
                    ds.getString(Tags.PersonName));
        }
        ...
        if ("TCOORD".equals(type)) {
            ...
            if ("END".equals(rangeType)) {
                return new TCoordContentImpl.End(owner, obsDateTime,
                        template, name, pos);
            }
    ...
    Content clone(KeyObject newOwner, boolean deep,
            boolean inheritObsDateTime) {
        Content clone = clone(newOwner, inheritObsDateTime);
        if (deep) {
            for (ContentImpl child = firstChild; child != null;
                    child = child.next) {
                clone.appendChild(child.relation,
                        child.clone(newOwner, true, false));
            }
        }
        return clone;
    }

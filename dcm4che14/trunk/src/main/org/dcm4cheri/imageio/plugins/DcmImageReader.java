...
public class DcmImageReader extends javax.imageio.ImageReader {
    ...
    private static final int TAG_PIXELDATA = 0x7FE00010;
    ...
    private void readMetadata() throws IOException {
        ...
        theParser.parseDcmFile(fileFormat, TAG_PIXELDATA);
        this.theMetadata = new DcmMetadataImpl(theDataset);        
        if (theParser.getReadTag() != TAG_PIXELDATA) {
    ...
    public Iterator getImageTypes(int imageIndex) throws IOException {
        ...
        if ("RGB".equals(this.pmi)) {
            ...
            l.add(new ImageTypeSpecifier(
                cmFactory.getColorModel(cmFactory.makeParam(theDataset)),
    ...
    private void readByteSamples(int samples, byte[] dest) throws IOException {
        ...
        try {
            for (y = 0; y < sourceHeight; ++y) {
                ...
                if (sourceXSubsampling == 1) {
                    stream.readFully(dest, destY*totDestRowLen + destXOffsetLen,
                            srcRowLen);
                }
                else {

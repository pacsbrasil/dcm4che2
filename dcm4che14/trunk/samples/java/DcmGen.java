...
/**
 * 
 * @author arnold.maderthaner@j4care.com
 */
public class DcmGen {
    ...
    private static ResourceBundle messages = ResourceBundle.getBundle("DcmGen",
            Locale.getDefault());
    ...
    public static void main(String args[]) throws Exception {
        ...
        Configuration cfg = new Configuration(DcmGen.class
                .getResource("dcmgen.cfg"));
    ...
    DcmGen(Configuration cfg, DcmURL url, int argc) {
        ...
        this.packPDVs = "true".equalsIgnoreCase(cfg.getProperty("pack-pdvs",
                "false"));
        this.truncPostPixelData = "true".equalsIgnoreCase(cfg.getProperty(
                "trunc-post-pixeldata", "false"));
        this.excludePrivate = "true".equalsIgnoreCase(cfg.getProperty(
                "exclude-private", "false"));
        this.bufferSize = Integer.parseInt(cfg.getProperty("buf-len", "2048")) & 0xfffffffe;
        this.repeatWhole = Integer.parseInt(cfg
                .getProperty("repeat-assoc", "1"));
        this.repeatSingle = Integer.parseInt(cfg.getProperty("repeat-dimse",
                "1"));
        this.uidSuffix = cfg.getProperty("uid-suffix");
        this.mode = argc > 1 ? SEND : ECHO;
        this.random = "true".equalsIgnoreCase(cfg.getProperty("set-random",
                "false"));
        this.complete = "true".equalsIgnoreCase(cfg.getProperty("set-complete",
                "false"));
    ...
    // Public --------------------------------------------------------
    public void execute(String[] args, int offset) throws InterruptedException,
    ...
    public void echo() throws InterruptedException, IOException,
        ...
        for (int i = 0; i < repeatWhole; ++i) {
            ...
            if (active != null) {
                if (active.getAssociation().getAcceptedTransferSyntaxUID(
                        PCID_ECHO) == null) {
        ...
        log.info(MessageFormat.format(messages.getString("echoDone"),
                new Object[] { new Integer(count), new Long(dt) }));
    }

    public void send(String[] args, int offset) throws InterruptedException,
            IOException, GeneralSecurityException {
        if (bufferSize > 0) {
            ...
            if (active != null) {
                for (int k = offset; k < args.length; ++k) {
                    ...
                    if (random) {
                        ...
                        if (ds == null)
                            ds = overwrite;
                    } else if (datasets.length > (k - offset)
                            && datasets[(k - offset)] != null) {
        ...
        long dt = System.currentTimeMillis() - t1;
        log.info(MessageFormat.format(messages.getString("sendDone"),
                new Object[] { new Integer(sentCount), new Long(sentBytes),
                        new Long(dt), new Float(sentBytes / (1.024f * dt)), }));
    }
    ...
    private boolean sendFile(ActiveAssociation active, File file,
            Dataset mergeDataset) throws InterruptedException, IOException {
        InputStream in = null;
            ...
            try {
                ...
                try {
                    ...
                    if (parser.getReadTag() == Tags.PixelData) {
                        if (parser.getStreamPosition() + parser.getReadLength() > file
                                .length()) {
                    ...
                    log.info(MessageFormat.format(messages
                            .getString("readDone"), new Object[] { file }));
                } catch (DcmParseException e) {
                    log.error(MessageFormat.format(messages
                            .getString("failformat"), new Object[] { file }));
                    return false;
                }
            } catch (IOException e) {
                log.error(MessageFormat.format(messages.getString("failread"),
                        new Object[] { file, e }));
                return false;
            }
    ...
    private boolean sendDataset(ActiveAssociation active, File file,
            DcmParser parser, Dataset ds) throws InterruptedException,
            IOException {
        ...
        if (sopInstUID == null) {
            log.error(MessageFormat.format(messages.getString("noSOPinst"),
                    new Object[] { file }));
            return false;
        }
        String sopClassUID = ds.getString(Tags.SOPClassUID);
        if (sopClassUID == null) {
            log.error(MessageFormat.format(messages.getString("noSOPclass"),
                    new Object[] { file }));
            return false;
        }
        PresContext pc = null;
        ...
        if (parser.getDcmDecodeParam().encapsulated) {
            ...
            if ((pc = assoc.getAcceptedPresContext(sopClassUID, tsuid)) == null) {
                log.error(MessageFormat.format(
                        messages.getString("noPCStore3"), new Object[] {
            ...
            log.error(MessageFormat.format(messages.getString("noPCStore2"),
                    new Object[] { uidDict.lookup(sopClassUID), file }));
            return false;

        }
        active.invoke(aFact.newDimse(pc.pcid(), oFact.newCommand()
                .initCStoreRQ(assoc.nextMsgID(), sopClassUID, sopInstUID,
                        priority), new MyDataSource(parser, ds, buffer)), null);
    ...
    private void applyUIDSuffix(Dataset ds) {
        ds.putUI(Tags.StudyInstanceUID, ds.getString(Tags.StudyInstanceUID, "")
                + uidSuffix);
        ds.putUI(Tags.SeriesInstanceUID, ds.getString(Tags.SeriesInstanceUID,
                "")
                + uidSuffix);
        ds.putUI(Tags.SOPInstanceUID, ds.getString(Tags.SOPInstanceUID, "")
                + uidSuffix);
    }
    ...
    private final class MyDataSource implements DataSource {
        ...
        public void writeTo(OutputStream out, String tsUID) throws IOException {
            DcmEncodeParam netParam = (DcmEncodeParam) DcmDecodeParam
                    .valueOf(tsUID);
            if (excludePrivate)
            ...
            if (parser.getReadTag() == Tags.PixelData) {
                ...
                ds.writeHeader(out, netParam, parser.getReadTag(), parser
                        .getReadVR(), parser.getReadLength());
                if (netParam.encapsulated) {
                    parser.parseHeader();
                    while (parser.getReadTag() == Tags.Item) {
                        ds.writeHeader(out, netParam, parser.getReadTag(),
                                parser.getReadVR(), parser.getReadLength());
                    ...
                    ds.writeHeader(out, netParam, Tags.SeqDelimitationItem,
                            VRs.NONE, 0);
                } else {
    ...
    private final void initAssocParam(Configuration cfg, DcmURL url,
        ...
        for (Enumeration it = cfg.keys(); it.hasMoreElements();) {
            ...
            if (key.startsWith("pc.")) {
                initPresContext(Integer.parseInt(key.substring(3)), cfg
                        .tokenize(cfg.getProperty(key), new LinkedList()));
            }
        }
    }
            ...
            if (key.startsWith("set.")) {
                ...
                try {
                    ...
                    throw new IllegalArgumentException(
                            "Illegal entry in DcmGen.cfg - " + key + "="
                                    + cfg.get(key));
                }
            }
        }

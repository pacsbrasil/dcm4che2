		...
		for(var x=0; x<rowIndex+1;x++) {
			...
			for(var y=0;y<colIndex+1;y++) {
                ...
                if(cnt < seriesData.length) {
                	if(currSer!= '' && currSer == (seriesData[cnt])['seriesUID']) {
                		divContent += 'src="frameContent.html?seriesUID=' + (seriesData[cnt])['seriesUID'] + '&instanceNumber=' + parent.imgNo + '"';
                	} else {
                		divContent += 'src="frameContent.html?seriesUID=' + (seriesData[cnt])['seriesUID'] + '"';
                	}
                }
		...
		for(var x=0;x<rowIndex+1;x++) {
			...
			for(var y=0; y<colIndex+1; y++) {
                ...
                if(cnt<seriesData.length){
                	...
                	divContent+= '&seriesUID=' + (seriesData[cnt])['seriesUID'] + '"';
                }
                divContent += ' style="background:#000"></iframe></td>';
                cnt = cnt + 1;
			}

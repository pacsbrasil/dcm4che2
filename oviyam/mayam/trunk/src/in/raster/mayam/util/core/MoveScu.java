 ...
 *
 * The Initial Developer of the Original Code is TIANI Medgraph AG. Portions
 * created by the Initial Developer are Copyright (C) 2002-2005 the Initial
 * Developer. All Rights Reserved.
 *
 * Contributor(s): Gunter Zeilinger <gunter.zeilinger@tiani.com>
 *
...
public class MoveScu {
        ...
        boolean applicable(Association as) {
            ...
            if (keys.contains(Tags.PatientID)) {
                ...
                if (keys.contains(Tags.StudyInstanceUID)) {
                    ...
                    if (keys.contains(Tags.SeriesInstanceUID)) {
                        return true;
                    }
                }
            }
            ExtNegotiation extNeg = as.getAAssociateAC().getExtNegotiation(cuid);
    ...
    private static ResourceBundle messages = ResourceBundle.getBundle("MoveScu", Locale.getDefault());
    ...
    public static void main(String args[]) throws Exception {
        ...
        Configuration cfg = new Configuration(MoveScu.class.getResource("/resources/movescu.cfg"));
        String pid = null;
        String issuer = null;
    ...
    private boolean initKeys(String pid, String issuer, String[] suids,
        ...
        if (serUids.length > 0) {
            if (suids.length > 1) {
                throw new IllegalArgumentException(messages.getString("erruids"));
            }
            level = "SERIES";
    ...
    private void move(ActiveAssociation a) throws Exception {
        ...
        if (pcid > 0) {
            Command cmd = oFact.newCommand();
            String cuid = as.getProposedPresContext(pcid).getAbstractSyntaxUID();
    ...
    private final void initAssocParam(Configuration cfg, DcmURL url) {
        ...
        assocRQ.addPresContext(aFact.newPresContext(PCID_PSO_MOVE,
                UIDs.PatientStudyOnlyQueryRetrieveInformationModelMOVE,
                tsUIDs));
        final boolean extNeg = "true".equalsIgnoreCase(cfg.getProperty("ext-neg", "true"));
        if (extNeg) {
            assocRQ.addExtNegotiation(aFact.newExtNegotiation(UIDs.PatientRootQueryRetrieveInformationModelMOVE,
                    RELATIONAL));
            assocRQ.addExtNegotiation(aFact.newExtNegotiation(UIDs.StudyRootQueryRetrieveInformationModelMOVE,
                    RELATIONAL));
            assocRQ.addExtNegotiation(aFact.newExtNegotiation(UIDs.PatientStudyOnlyQueryRetrieveInformationModelMOVE,
                    RELATIONAL));
        }
    }
    ...
    private void initTLS(Configuration cfg) {
        try {
            ...
            tls.setKey(tls.loadKeyStore(MoveScu.class.getResource(cfg.getProperty("tls-key", "identity.p12")), keypasswd),
                    keypasswd);
            tls.setTrust(tls.loadKeyStore(MoveScu.class.getResource(cfg.getProperty("tls-cacerts", "cacerts.jks")), cfg.getProperty("tls-cacerts-passwd", "secret").toCharArray()));
            tls.init();
        } catch (Exception ex) {

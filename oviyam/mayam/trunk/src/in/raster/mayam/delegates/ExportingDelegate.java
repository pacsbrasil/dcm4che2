...
import in.raster.mayam.form.dialogs.ExportLocationChooser;
...
public class ExportingDelegate extends Thread {
    ...
    private void process(ExportToDcmDelegate exportToDcmDelegate, JpegConvertorDelegate jpegDelegate) {
        if (isSeriesOrInstanceLevel) {
            File patientNameFile = null;
            if (!ApplicationContext.imgView.getImageToolbar().isImageLayout) {
                patientNameFile = new File(openedFile, ApplicationContext.layeredCanvas.imgpanel.getTextOverlayParam().getPatientName());
            } else {
                patientNameFile = new File(openedFile, ((LayeredCanvas) ApplicationContext.selectedPanel.getComponent(0)).imgpanel.getTextOverlayParam().getPatientName());
            }
    ...
    private void exportSeriesOfImage(File patientNameFile, ExportToDcmDelegate exportToDcmDelegate, JpegConvertorDelegate jpegDelegate) {
        if (ExportDialog.conversionFormatDcm) {
            exportToDcmDelegate.seriesExportAsDicom(ApplicationContext.layeredCanvas.imgpanel.getStudyUID(), ApplicationContext.layeredCanvas.imgpanel.getSeriesUID(), ApplicationContext.layeredCanvas.imgpanel.isMultiFrame(), ApplicationContext.layeredCanvas.imgpanel.getInstanceUID(), patientNameFile.getAbsolutePath());
        } else {
            jpegDelegate.seriesLevelConvertor(ApplicationContext.layeredCanvas.imgpanel.getStudyUID(), ApplicationContext.layeredCanvas.imgpanel.getSeriesUID(), ApplicationContext.layeredCanvas.imgpanel.isMultiFrame(), ApplicationContext.layeredCanvas.imgpanel.getInstanceUID(), patientNameFile.getAbsolutePath(), ApplicationContext.layeredCanvas.imgpanel.getColorModel());
        }
    }
    ...
    private void exportInstanceOfImage(File patientNameFile, ExportToDcmDelegate exportToDcmDelegate, JpegConvertorDelegate jpegDelegate) {
        if (ExportDialog.conversionFormatDcm) {
            exportToDcmDelegate.instanceExportAsDicom(ApplicationContext.layeredCanvas.imgpanel.getDicomFileUrl(), patientNameFile.getAbsolutePath(), ApplicationContext.layeredCanvas.imgpanel.isEncapsulatedDocument());
        } else {
            jpegDelegate.instanceConvertor(patientNameFile.getAbsolutePath() + File.separator + ApplicationContext.layeredCanvas.imgpanel.getInstanceUID(), ApplicationContext.layeredCanvas.imgpanel.getCurrentbufferedimage());
        }
    }

...
public class ReceiveDelegate {
    ...
    public ReceiveDelegate() {
        dcmrcv = new DcmRcv();
        dcmrcv.setAEtitle(ApplicationContext.listenerDetails[0]);
        dcmrcv.setHostname("0.0.0.0");
        dcmrcv.setPort(Integer.parseInt(ApplicationContext.listenerDetails[1]));
        this.port = Integer.parseInt(ApplicationContext.listenerDetails[1]);
        dcmrcv.setDestination(ApplicationContext.listenerDetails[2]);
        dcmrcv.setPackPDV(false);
        dcmrcv.setTcpNoDelay(false);
        dcmrcv.initTransferCapability();
        dcmrcv.setTlsNeedClientAuth(false);
        MoveScu.maskNull(ApplicationContext.listenerDetails[0]);
    }

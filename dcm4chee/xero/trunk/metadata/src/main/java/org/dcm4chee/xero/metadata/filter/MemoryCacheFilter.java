...
public class MemoryCacheFilter<T> implements Filter<T>, MetaDataUser {
   ...
   /** The default item to lookup in params to find the key for the cached item. */
	...
	/** The metadata item to define how long to keep the object open for - default to zero */
	public static final String CLOSE_AGE="closeAge";
	
	/** The value to use for cache-control to NOT cache the item */
	public static final String NO_CACHE="no-cache";
	
	/** The key to use in the meta-data for the size, and in the params to denote the size of the cached item. */
   ...
   public T filter(FilterItem filterItem, Map<String, Object> params) {
	  ...
	  log.info(msg);
	  if( dur > 1000000000 ) log.warn("More than 1 second to fetch "+key);
	  return ret;
   }
	...
	public void setMetaData(MetaDataBean metaDataBean) {
		...
		if( cacheSize!=null ) setCacheSize(Long.parseLong(cacheSize));
		cacheName = metaDataBean.getPath();
		closeAge = (String) metaDataBean.getValue(CLOSE_AGE);
		if( closeAge!=null ) cache.setCloseAge(Long.parseLong(closeAge));
	}
  ...
  /**
   ...
   /** 
    * Get a future item from the next filter item, and be able to say how bit it is.
    * @author bwallace
    *
    * @param <T>
    */

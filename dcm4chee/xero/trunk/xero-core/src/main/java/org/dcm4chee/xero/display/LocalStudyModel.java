...
public class LocalStudyModel {
	...
	public String getPatientXml() {
		...
		if( pid!=null && !children.containsKey(pid) ) return "";
		if (clearEmpty())
			return "";
		try {
			// TODO - figure out how to get a complete list of recognized classes
			...
			if( xmlIndicator>=0 ) ret = ret.substring(xmlIndicator+2);
	...
	/**
	 * Gets the customizations for the study specific data - this is worthwhile from the
	 * point of view of displaying the object, since it minimizes how much customized data
	 * is found.
	 * @return
	 */
	public String getStudyXml() {
		String studyUid = studyLevel.getStudyUID();
		if( studyUid==null ) throw new IllegalArgumentException("No Study UID provided.");
		// If no customizations for this item, return immediately.
		...
		try {
			// TODO - figure out how to get a complete list of recognized classes
			// so that serialization works correctly.
			JAXBContext context = JAXBContext.newInstance(StudyBean.class);
			Marshaller m = context.createMarshaller();
			StringWriter sw = new StringWriter();
	...
	public StudyBean getStudy() {
		if (studyLevel == null)
			throw new IllegalArgumentException(
					"StudyLevel information must be supplied.");
		String studyUid = studyLevel.getStudyUID();
		if (studyUid == null)
	...
	public ImageBean getImage() {
		...
		ImageBean ret = (ImageBean) children.get(objectUid + ","
				+ studyLevel.getFrame());
		if (ret != null)
			return ret;
	...
	public boolean clearEmpty() {
		...
		if (isEmpty) {
			...
			children.remove(new PatientIdentifier(patient

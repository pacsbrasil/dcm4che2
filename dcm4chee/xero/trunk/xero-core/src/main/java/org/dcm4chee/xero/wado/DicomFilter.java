...
public class DicomFilter implements Filter<DicomObject> {
   ...
   private static final StopTagInputHandler stopHandler = new StopTagInputHandler(Tag.PixelData);
   ...
   public DicomObject filter(FilterItem filterItem, Map<String, Object> params) {
	  ...
	  if (fullReadStr != null)
		 fullRead = Boolean.parseBoolean(fullReadStr);
	  ...
	  if (location == null)
		 return null;
	  try {
		 DicomInputStream dis = new DicomInputStream(new BufferedInputStream(location.openStream()));
		 if (!fullRead)
	  ...
	  if (ret == null)
		 return (DicomObject) filterItem.callNextFilter(params);
	  return ret;
   }

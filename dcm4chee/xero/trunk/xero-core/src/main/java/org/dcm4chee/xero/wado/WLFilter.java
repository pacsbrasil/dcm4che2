...
/**
 * This class applies the DICOM window level operation directly to the pixel data transform, so 
 * that no new copy of the data is required.
 * @author bwallace
 *
 */
public class WLFilter implements Filter<WadoImage> 
{
	...
	public WadoImage filter(FilterItem filterItem, Map<String, Object> params) {
		...
		// The lookup op, either computed by hand, or retrieved from the colour model.
	...
	/** This version of compute lookup op generates a table based on the indexed colour model. */

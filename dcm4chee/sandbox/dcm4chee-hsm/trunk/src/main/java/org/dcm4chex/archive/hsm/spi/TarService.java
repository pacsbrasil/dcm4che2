...
public class TarService extends ServiceMBeanSupport implements TarArchiver {
    ...
    private static final String WRONG_BUFFER_SIZE = "Wrong buffer size: {0}"; // NON-NLS
    static final String MD5_SUM = "MD5SUM"; // NON-NLS
    public static final String UNEXPECTED_TAR_ENTRY = "Unexpected tar entry [{0}]. Digest check failed."; // NON-NLS
    private static final String PACKING_FILES_INTO_A_TAR_FILE = "Packing files into a tar file"; // NON-NLS
    private static final String TAR_SUFFIX = ".tar"; // NON-NLS
    private static final int SIZE_OF_ADDITIONAL_CHARS = 6;
    private static final String NOTHING_TO_PACK_INTO_TAR = "Nothing to pack into tar, passed files were: [{0}]"; // NON-NLS
    private static final String COULD_NOT_DELETE_TAR_FILE = "Could not delete tar file [{0}]"; // NON-NLS
    private static final String WARNING_TAR_FILE_EXISTS = "Tar file exists, will delete it [{0}]"; // NON-NLS
    private static final String US_ASCII = "US-ASCII"; // NON-NLS
    private static final String COULD_NOT_CLOSE_FILE_INPUT_STREAM = "Could not close file input stream [{0}]"; // NON-NLS
    private static final String COULD_NOT_CLOSE_TAR_OUTPUT_STREAM = "Could not close TAR output stream [{0}]"; // NON-NLS
    static final String FAILED_DIGEST_CHECK = "Failed {0} check of tar entry [{1}], expected [{2}], was [{3}]"; // NON-NLS
    ...
    private static class TarExtractor {
        ...
        private static final String MISSING_MD_SUMS_ENTRY = "Missing MD5SUMS entry in [{0}]"; // NON-NLS
        private static final int NAME_BEGIN_INDEX = 34;
        private static final int MD5_END_INDEX = 32;
        private static final String M_WRITE = "M-WRITE: [{0}]"; // NON-NLS
        private static final String M_DELETE = "M-DELETE: [{0}]"; // NON-NLS
        private static final String CLEANING_UP = "Cleaning up all extracted files of last tar file due to exception."; // NON-NLS
        private static final String COULDNT_DELETE_FILE = "Could not delete file [{0}]"; // NON-NLS
        private static final String FILE_EXISTS_WILL_OVERWRITE_IT = "File exists: [{0}], will overwrite it."; // NON-NLS
        private static final String COULDNT_DELETE_PARENT_FOLDERS = "Could not delete parent folders of [{0}]."; // NON-NLS
        private static final String TAR_UNPACKING = "TAR - Unpacking [{0}]"; // NON-NLS
        ...
        TarExtractor(File tar, String destDir, int bufferSize, boolean checkMd5) throws Exception {
            ...
            if (this.checkMd5) {
                ...
                while ((line = reader.readLine()) != null) {
                    this.md5Sums.put(line.substring(NAME_BEGIN_INDEX),
                            MD5Utils.toBytes(line.substring(0, MD5_END_INDEX)));
                }
            }
        }
        ...
        private DigestVerifyingOutputStream newDigestVerifyingOS(File file,
            ...
            return new DigestVerifyingOutputStream(new FileOutputStream(file),
                    expectedDigest,
                    entryName);
        }
    ...
    private static class DigestVerifyingOutputStream extends DigestOutputStream {
        private static final String MD5 = "MD5"; // NON-NLS

        
        private final byte[] expectedDigest;
        private final String entryName;

        public DigestVerifyingOutputStream(OutputStream stream,
                                           byte[] expectedDigest,
                                           String entryName) throws Exception {
        ...
        public void close() throws IOException {
            ...
            if (!MessageDigest.isEqual(expectedDigest, actualDigest)) {
                throw new FailedDigestCheckException(MessageFormat.format(FAILED_DIGEST_CHECK,
                        getMessageDigest().getAlgorithm(),
                        entryName,
                        toHexStr(expectedDigest),
                        toHexStr(actualDigest)));
            }
        }

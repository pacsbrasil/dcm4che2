...
{ 
  ...
  */
  ...
  /* NOT YET IMPLEMENTED
      ...
      The structure modifier controls whether we output lazy, eager, or
      normalized structure. Since String constants are always intern()ed
      we simply make sure that the _structure variable also gets interned
      so we can make the comparisons as cheap as int comparisons.

  COMMENTED OUT BECAUSE IT'S NOT READY YET

  private String _structure;
  public static final String STRUCTURE_LAZY 
                             = "java:org.regenstrief.xhl7.structure.lazy";
  public static final String STRUCTURE_LAZY 
                             = "java:org.regenstrief.xhl7.structure.lazy";
  public static final String STRUCTURE_LAZY 
                             = "java:org.regenstrief.xhl7.structure.lazy";
  ...
  */
    ...
    throws IOException, SAXException 
  {
    ...
    this._contentHandler.startElement(NAMESPACE_URI,
				      TAG_ROOT, TAG_ROOT, this._atts);
	...
	if(segmentTag == "FHS" || segmentTag == "BHS" || segmentTag == "MSH") {
	  ...
	  this._atts.addAttribute(NAMESPACE_URI, 
				  ATT_DEL_FIELD, ATT_DEL_FIELD,
				  CDATA, delimiters.substring(0,1));
	  this._atts.addAttribute(NAMESPACE_URI, 
				  ATT_DEL_COMPONENT, ATT_DEL_COMPONENT,
				  CDATA, delimiters.substring(1,2));
	  this._atts.addAttribute(NAMESPACE_URI, 
				  ATT_DEL_REPEAT, ATT_DEL_REPEAT,
				  CDATA, delimiters.substring(2,3));
	  this._atts.addAttribute(NAMESPACE_URI, 
				  ATT_DEL_ESCAPE, ATT_DEL_ESCAPE,
				  CDATA, delimiters.substring(3,4));
	  this._atts.addAttribute(NAMESPACE_URI, 
				  ATT_DEL_SUBCOMPONENT, ATT_DEL_SUBCOMPONENT,
				  CDATA, delimiters.substring(4,5));
	} else {
	...
	while(tokens.hasMoreTokens()) {
	  ...
	  if(token.length() == 1) {
	    ...
	    case N_DEL_FIELD: newElement(N_FIELD); continue;
	    case N_DEL_REPEAT: newElement(N_REPEAT); continue;
	    case N_DEL_COMPONENT: newElement(N_COMPONENT); continue;
	    case N_DEL_SUBCOMPONENT: newElement(N_SUBCOMPONENT); continue;
	    case N_DEL_ESCAPE:
	      /*
		I have seen too many spurious backslashes in messages
		and it's much harder to fix this after the fact.
		(field misalignments are easier to fix and harder
		to detect here.) So, we only report escapes that look
		like they could be a valid escape sequence. Some 
		heuristics are (1) a single upper case character /A/, 
		(2) a sequence starting with a period /.bla/ (3)
		an upper case character followed by a non alpha character
		/U+0382/. Expressly a leading digit will be rejected
		as an escape sequence. The lookAhead(2) must be an 
		escape character also, and if the lookAhead(3) looks 
		like a known escape sequence, we will ignore the present
		escape and pick the next.
	      */
	      String closingEscape = tokens.lookAhead(2);

	      if(closingEscape == null)
		break;
	      if(closingEscape.length() != 1)
		break;
	      if(closingEscape.charAt(0)!=delimiters.charAt(N_DEL_ESCAPE))
		break;
	      
	      if(likeEscapeContent(tokens.lookAhead(1)) 
		 < likeEscapeContent(tokens.lookAhead(3)))
		break;

	      toggleEscape();
	      token = tokens.nextToken();
	      this._contentHandler.characters(token.toCharArray(), 0, 
					      token.length());
	      token = tokens.nextToken();
	      toggleEscape();
	      continue;
    ...
    this._contentHandler.endElement(NAMESPACE_URI,
				    TAG_ROOT, TAG_ROOT);
  ...
  */
  int likeEscapeContent(String content) {
    ...
    if(length == 1) {
      ...
      case 'R': return 100;
      case 'E': return 100;
      default: return (('A' < c0 && c0 < 'Z') ? 10 : -10);
      }
    } else if(length < 10) {
      switch(c0) {
      case '.':
  ...
  static public class Tokenizer {
    ...
    static public int BUFFER_SIZE = 8;
	...
	String token = 
	  (this._tokens.hasMoreTokens() ? this._tokens.nextToken() : null);
	...
	one. */
    String nextToken() {
      this._ringBuffer[this._bufferPointer] 
	= (this._tokens.hasMoreTokens() ? this._tokens.nextToken() : null);
    ...
    */
	...
	throw new IllegalArgumentException("lookahead by more than " 
					   + BUFFER_SIZE);
    }
  }


  /* The state machine to keep track of which elements to close. */
  ...
  private static String tagName[] = {
    TAG_ESCAPE, TAG_SUBCOMPONENT, TAG_COMPONENT, TAG_REPEAT, TAG_FIELD
  };
  
  private void closeElement(int level) 
    throws SAXException
  {
  ...
  private void newElement(int level) 
    throws SAXException
  {
    closeElement(level);
    this._contentHandler.startElement(NAMESPACE_URI, 
				      tagName[level], 
				      tagName[level], this._atts);
  ...
  */
    ...
    throws SAXNotRecognizedException, SAXNotSupportedException
  {
    /* NOT READY YET
      name = name.intern();
    if(name == STRUCTURE_LAZY || name == STRUCTURE_EAGER ||
  ...
  */
  ...
  { 
	...
	this._structure = STRUCTURE_LAZY;
     } else 
    */  
  ...
  /* This is all more or less irrelevant stuff from the XMLReader
  ...
  */
    ...
    throws Exception 
  {
	...
	transformer.transform(new SAXSource(reader, new InputSource(args[0])),

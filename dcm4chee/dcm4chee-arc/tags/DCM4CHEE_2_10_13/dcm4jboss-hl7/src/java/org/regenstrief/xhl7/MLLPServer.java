/*  HL7 MLLP Server calling XSLT transform on an input message.
 *
...
public class MLLPServer extends Thread {
  ...
  private static Map/*<String, MLLPServer>*/ _serverNameMap
    = new IdentityHashMap/*<String, MLLPServer>*/();
  ...
  private MLLPServer(String id) {
    ...
    id = id.intern();
    if(MLLPServer._serverNameMap.containsKey(id))
  ...
  private static class ConfigurationHandler implements ContentHandler {
	...
	if(_currentServer == null)
	  throw new SAXException(localName + " must be in a "
				 + TAG_SERVER
				 + " element");
	...
	if(portNumberString != null)
	  ...
	  throw new SAXException(localName + " must have a "
				 + ATT_PORT_NUMBER
				 + " attribute");
	...
	if(className == null)
	  throw new SAXException(localName + " must have a "
				 + ATT_TRANSFORMER_FACTORY_CLASS
				 + " attribute");

	try {
	  this._currentTransformerFactory = 
	...
	if(this._currentTransformerFactory == null)
	  throw new SAXException(localName + " must follow a "
				 + TAG_TRANSFORMER_FACTORY 
				 + " element");
	...
	try {
	  Source transformSource = new StreamSource(
		(new URL(href)).openStream());	  
	  this._currentServer.setTemplates(
	    this._currentTransformerFactory.newTemplates(transformSource));
	} catch(Exception ex) {
	  throw new SAXException(ex);
	}
	...
	try {
	  Source transformSource = new StreamSource(
		                      (new URL(href)).openStream());	  
	  this._currentServer.setErrorTemplates(
	      this._currentTransformerFactory.newTemplates(transformSource));
	} catch(Exception ex) {
	  throw new SAXException(ex);
	}
	...
	// start the server, will spawn a new tread for that server
  ...
  public void run() {
    ...
    try {
      acceptor 
	= new ServerSocket(this._portNumber, this._backlog, null);
    } catch(Exception ex) {
      throw new Error(ex);
    }

    System.err.println("["+ this._id +"] accepting connections on " 
		       + acceptor.getInetAddress() + " "
		       + acceptor.getLocalPort());
    
    while(true /* and no shutdownrequest signaled */) { 
	...
	// FIXME: need some rate limit and suspension for bad handlers
      }
    }  
  }
  ...
  private class ClientHandler extends Thread {
    ...
    */
    ...
    public void run() {
      System.err.println("["+ _id +"] serving " 
			 + this._connection.getRemoteSocketAddress());

      try {
	// create a new transformer working context
	Transformer transformer = _templates.newTransformer();
	transformer.setParameter(PAR_REMOTE_ADDR, 
				this._connection.getRemoteSocketAddress());
	
	MLLPDriver mllpDriver 
	  = new MLLPDriver(this._connection.getInputStream(),
	...
	if(_readerClass != null)
	  source = new SAXSource((XMLReader)_readerClass.newInstance(),
				 new InputSource(mllpDriver.getInputStream()));
	else
	  source = new StreamSource(mllpDriver.getInputStream());
	...
	while(mllpDriver.hasMoreInput()) {
	  try {
	    ...
	    if(_errorTemplates != null) {
	      // create a new error transformer working context
	      Transformer errorTransformer = _errorTemplates.newTransformer();
	      errorTransformer.setParameter(PAR_SERVER_ID, _id);
	      errorTransformer.setParameter(PAR_REMOTE_ADDR, 
			     this._connection.getRemoteSocketAddress());
	      errorTransformer.setParameter(PAR_EXCEPTION, ex);
	      errorTransformer.transform(
                   new StreamSource(
                         new ByteArrayInputStream("<null/>".getBytes())), 
		   result);	
	      mllpDriver.turn();
	    }
	  }
	  System.err.println("["+ _id +"] message done from "
			     + this._connection.getRemoteSocketAddress());
	}

	System.err.println("["+ _id +"] server exit from "
			   + this._connection.getRemoteSocketAddress());
      } catch(Exception ex) {

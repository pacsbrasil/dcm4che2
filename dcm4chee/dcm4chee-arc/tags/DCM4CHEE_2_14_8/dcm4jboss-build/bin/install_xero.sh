#!/bin/sh
# -----------------------------------------------------------------
# copy XERO and WADO2 components into DCM4CHEE Archive installation
# -----------------------------------------------------------------
...
if [ ! -f "$XER_SERV"/deploy/xero.war ]; then
...
cp -v "$XER_SERV"/deploy/xero.war "$DCM4CHEE_SERV"/deploy/
cp -v "$XER_SERV"/deploy/jboss-web.deployer/server.xml \

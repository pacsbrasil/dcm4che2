...
class StgCmtCmd {
    ...
    private static final DcmObjectFactory objFact = DcmObjectFactory
            .getInstance();
    ...
    public StgCmtCmd(StoreScpService service, Association a, Dataset actionInfo)
        ...
        if (uid == null) { throw new DcmServiceException(
        ...
        if (refSOPSeq == null) { throw new DcmServiceException(
                Status.MissingAttributeValue,
    ...
    public void execute() {
        ...
        for (int i = 0, n = refSOPSeq.vm(); i < n; ++i) {
            ...
            if (storage != null
                    && (failureReason = commit(storage, refSOP)) == Status.Success) {
    ...
    private int commit(Storage storage, Dataset refSOP) {
        ...
        if (fileInfo == null) {
            service.getLog().warn(
                    "Failed Storage Commitment of Instance[uid=" + iuid
                            + "]: no such object");
        ...
        try {
            ...
            service.getLog().error(
                    "Failed Storage Commitment of Instance[uid="
    ...
    private FileInfo[] findByIUID(String iuid) {
        for (int i = 0; i < fileInfos.length; i++) {
            if (fileInfos[i][0].sopIUID.equals(iuid)) { return fileInfos[i]; }
        }
        return null;
    }
    ...
    private void checkFile(FileInfo info) throws IOException {
        ...
        byte[] md5 = md.digest();
        if (!Arrays.equals(md5, info.getFileMd5())) { throw new IOException(
                "MD5 mismatch"); }
    }
    ...
    private void sendResult() {
        try {
            ...
            ActiveAssociation activeAssoc = aFact.newActiveAssociation(assoc,
                    null);
            activeAssoc.start();
            if (checkAAssociateAC((AAssociateAC) pdu)) {
                ...
                service.getLog().error(
                        "storage commitment (requestor=SCP) rejected by "
                                + aeData);
            }
            try {
                ...
                service.getLog().warn(
                        "release association to " + aeData + " failed:", e);
            }
        } catch (Exception e) {
            service.getLog().error(
                    "sending storage commitment result to " + aeData
                            + " failed:", e);
        }
    }
    ...
    private AAssociateRQ makeAAssociateRQ() {
        ...
        rq.addPresContext(aFact.newPresContext(1,
                UIDs.StorageCommitmentPushModel, NATIVE_LE_TS));
        rq.addRoleSelection(aFact.newRoleSelection(

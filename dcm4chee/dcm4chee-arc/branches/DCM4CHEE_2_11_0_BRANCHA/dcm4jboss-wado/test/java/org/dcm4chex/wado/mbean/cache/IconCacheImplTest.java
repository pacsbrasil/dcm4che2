 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class IconCacheImplTest extends TestCase {

	static IconCache cache = IconCacheImpl.getInstance();

	public static void main(String[] args) {
		junit.textui.TestRunner.run(IconCacheImplTest.class);
	}
	...
	public void testGetIconFileDefault() throws Exception {
		...
		for ( int iStudy = 0 ; iStudy < iStudyMax ; iStudy++ ) {
			for ( int iSeries = 0 ; iSeries < iSeriesMax ; iSeries++ ) {
				for ( int iInstances = 0 ; iInstances < iInstancesMax ; iInstances++ ) {
					cache.putIcon(_getBI(),"study"+iStudy,
									   "series"+iSeries,
									   "instance"+iInstances);
				}
			}
		...
		for ( int iStudy = 0 ; iStudy < iStudyMax ; iStudy++ ) {
			for ( int iSeries = 0 ; iSeries < iSeriesMax ; iSeries++ ) {
				for ( int iInstances = 0 ; iInstances < iInstancesMax ; iInstances++ ) {
					File f = cache.getIconFile("study"+iStudy,"series"+iSeries,"instance"+iInstances);
		...
		File f = cache.getIconFile("study2not","series0","instance0");
		...
		for ( int iStudy = 0 ; iStudy < iStudyMax ; iStudy++ ) {
			for ( int iSeries = 0 ; iSeries < iSeriesMax ; iSeries++ ) {
				for ( int iInstances = 0 ; iInstances < iInstancesMax ; iInstances++ ) {
					for ( int iRows = 100 ; iRows < 400 ; iRows+=50 ) {
						for ( int iColumn = iRows ; iColumn < iRows+70 ; iColumn+=20 ) {
							File f = cache.getIconFile("study"+iStudy,
														"series"+iSeries,
														"instance"+iInstances,
														String.valueOf(iRows),
														String.valueOf(iColumn));
	...
	public void testClearCache() {
		try {
			long l = System.currentTimeMillis();
			cache.putIcon(_getBI(),"study0","series0","instance0");
			cache.putIcon(_getBI(),"study0","series0","instance1");
			cache.putIcon(_getBI(),"study0","series1","instance0");
			cache.putIcon(_getBI(),"study0","series1","instance1");
			cache.putIcon(_getBI(),"study1","series0","instance0");
			cache.clearCache();
			File f;
			f = cache.getIconFile("study0","series0","instance0");
			assertTrue("study0,series0,instance0 is in cache", f == null);
			f = cache.getIconFile("study0","series0","instance1");
	...
	private void _prepareCleanTest() {
		...
		try {
			for ( int iStudy = 0 ; iStudy < iStudyMax ; iStudy++ ) {
				studyDir = new File ( ((IconCacheImpl)cache).getCacheRoot(), "study"+iStudy );
		...
		( (IconCacheImpl) cache).setMinFreeSpace( currFree + l / 10 );
		( (IconCacheImpl) cache).setPreferredFreeSpace( currFree + l / 2 );

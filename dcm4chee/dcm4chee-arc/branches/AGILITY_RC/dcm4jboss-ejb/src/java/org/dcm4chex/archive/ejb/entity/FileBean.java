 ...
 * @ejb.finder signature="java.util.Collection findFilesToCompress(long fspk, java.lang.String cuid, java.sql.Timestamp before, java.sql.Timestamp after, int limit)"
 *             query="" transaction-type="Supports"
 * @jboss.query signature="java.util.Collection findFilesToCompress(long fspk, java.lang.String cuid, java.sql.Timestamp before, java.sql.Timestamp after, int limit)"
 *             query="SELECT OBJECT(f) FROM File AS f WHERE f.fileStatus = 0 AND f.fileTsuid IN ('1.2.840.10008.1.2','1.2.840.10008.1.2.1','1.2.840.10008.1.2.2') AND f.fileSystem.pk = ?1 AND f.instance.sopCuid = ?2 AND f.createdTime < ?3 AND f.createdTime > ?4 LIMIT ?5"
 *             strategy="on-find" eager-load-group="*"

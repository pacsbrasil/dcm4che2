 ...
 *             strategy="on-find" eager-load-group="*"
 * @ejb.finder signature="java.util.Collection findFilesOfSeriesOnFileSystemGroup(java.lang.String seriesIUID, java.lang.String fsgroupID)"
 *             query="SELECT OBJECT(f) FROM File AS f WHERE f.instance.series.seriesIuid = ?1 AND f.fileSystem.groupID = ?2" ORDER BY f.pk DESC"
 *             transaction-type="Supports"
 * @jboss.query signature="java.util.Collection findFilesOfSeriesOnFileSystemGroup(java.lang.String seriesIUID, java.lang.String fsgroupID)"
 *             strategy="on-find" eager-load-group="*"
 ...
 *             query="SELECT OBJECT(f) FROM File AS f WHERE f.fileStatus = 0 AND f.fileTsuid NOT IN ('1.2.840.10008.1.2.4.50','1.2.840.10008.1.2.4.51','1.2.840.10008.1.2.4.81','1.2.840.10008.1.2.4.91') AND f.fileSystem.status IN (0,1) AND f.fileSystem.groupID = ?1 AND f.instance.externalRetrieveAET = ?2 AND f.instance.sopCuid = ?3 AND f.instance.series.bodyPartExamined = ?4 AND f.instance.series.sourceAET = ?5 AND (f.createdTime IS NULL OR f.createdTime < ?6) LIMIT ?7"
 *             strategy="on-find" eager-load-group="*"
 ...
 *             query="SELECT DISTINCT OBJECT(f) FROM File AS f, IN (f.instance.files) AS f2 WHERE f.fileStatus = 0 AND f.fileTsuid NOT IN ('1.2.840.10008.1.2.4.50','1.2.840.10008.1.2.4.51','1.2.840.10008.1.2.4.81','1.2.840.10008.1.2.4.91') AND f.fileSystem.status IN (0,1) AND f.fileSystem.groupID = ?1 AND f2.fileSystem.groupID = ?2 AND f.instance.sopCuid = ?3 AND f.instance.series.bodyPartExamined = ?4 AND f.instance.series.sourceAET = ?5 AND (f.createdTime IS NULL OR f.createdTime < ?6) LIMIT ?7"
 *             strategy="on-find" eager-load-group="*"
 */

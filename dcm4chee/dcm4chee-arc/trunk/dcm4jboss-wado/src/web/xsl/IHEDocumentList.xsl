...
<xsl:template match="/">
    ...
    <body>
	    <h2><xsl:value-of select="IHEDocumentList/code/@displayName"/> for <xsl:value-of select="IHEDocumentList/recordTarget/patient/patientPatient/name/family"/>,
	    	<xsl:value-of select="IHEDocumentList/recordTarget/patient/patientPatient/name/given"/>
	    </h2>
        <p> 
        	<object>
                ...
                <xsl:call-template name="formatDate">
                	<xsl:with-param name="date" select="IHEDocumentList/recordTarget/patient/patientPatient/birthTime/@value"/>
                </xsl:call-template><br/>
	            <b> List Created:</b> 
				<xsl:call-template name="formatDateWithTime">
					<xsl:with-param name="dateTime" select="IHEDocumentList/activityTime/@value"/>

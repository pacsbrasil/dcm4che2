...
class StgCmtCmd {
    ...
    public StgCmtCmd(
        ...
        try {
            this.aeData =
                new AECmd(service.getDS(), a.getCallingAET()).execute();
            if (aeData == null) {
                throw new DcmServiceException(
                    Status.ProcessingFailure,
                    "Failed to resolve AET:" + a.getCallingAET());
            }
            this.fileInfos =
                RetrieveCmd.create(service.getDS(), refSOPSeq).execute();
    ...
    public void execute() {
        ...
        try {
            StorageHome home =
                (StorageHome) EJBHomeFactory.getFactory().lookup(
                    StorageHome.class,
    ...
    private void checkFile(FileInfo[] fileInfo) throws IOException {
        ...
        throw new IOException(
            "Instance not accessable from " + service.getAET());
    }
    ...
    private void sendResult() {
        try {
            ...
            if (checkAAssociateAC((AAssociateAC) pdu)) {
                ...
                service.getLog().error(
                    "storage commitment (requestor=SCP) rejected by " + aeData);
            }
            try {

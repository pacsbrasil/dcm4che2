...
public class AuditLoggerService extends ServiceMBeanSupport
         implements org.dcm4chex.service.AuditLoggerServiceMBean
{
    private final AuditLoggerFactory alf = AuditLoggerFactory.getInstance();
    private final AuditLogger logger = alf.newAuditLogger(
     ...
     * @jmx.managed-attribute
     */
    public void setSyslogHost(String newSyslogHost)
        throws Exception
    {
            ...
            Object oldVal, Object newVal, String type)
    {
        if (!newVal.equals(oldVal)) {
            logActorConfig(actorName + ": " + propName
    ...
    {
        if (getState() == STARTED) {
            logger.logActorConfig(desc,
                    alf.newLocalUser(getCurrentPrincipalName()),
                    type);
        }
    }
     ...
     * @jmx.managed-operation
     */
    public void logActorStartStop(String actorName, String action)
    {
        if (getState() == STARTED) {
            logger.logActorStartStop(actorName, action,
                    alf.newLocalUser(getCurrentPrincipalName()));
        }
    }
     ...
     * @jmx.managed-operation
     */
    public void logHandshakeFailedEvent(HandshakeFailedEvent event) {
        if (getState() == STARTED) {
            logger.logSecurityAlert("NodeAuthentification",

...
if [ -f "$ARR_SERV"/deploy/dcm4chee-arr-db2-3.0.5.ear ]; then
  ARR_DB=db2
elif [ -f "$ARR_SERV"/deploy/dcm4chee-arr-hsql-3.0.5.ear ]; then
  ARR_DB=hsql
elif [ -f "$ARR_SERV"/deploy/dcm4chee-arr-mssql-3.0.5.ear ]; then
  ARR_DB=mssql
elif [ -f "$ARR_SERV"/deploy/dcm4chee-arr-mysql-3.0.5.ear ]; then
  ARR_DB=mysql
elif [ -f "$ARR_SERV"/deploy/dcm4chee-arr-oracle-3.0.5.ear ]; then
  ARR_DB=oracle
elif [ -f "$ARR_SERV"/deploy/dcm4chee-arr-psql-3.0.5.ear ]; then
  ARR_DB=psql
else
...
cp -v "$ARR_SERV"/deploy/dcm4chee-arr-${ARR_DB}-3.0.5.ear \

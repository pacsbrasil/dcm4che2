...
public class KeyObjectBean extends KeyObjectType implements DicomObjectInterface, SearchableDicomObject {
    ...
    private void initKeySelectionRefs(DicomObject ds) {
        DicomElement sq = ds.get(Tag.ContentSequence);
        ...
        DicomElement evidence = ds.get(Tag.CurrentRequestedProcedureEvidenceSequence);
        if (evidence == null || !evidence.hasItems()) {
            log.warn("Key object document {} has no current requested procedure evidence sequence (0040,A375 DICOM code)", ds
                    .getString(Tag.SOPInstanceUID));
            return;
        }
        log.info("Adding series/study UID data.");
        Map<String, String> seriesMap = new HashMap<String, String>();
        Map<String, String> studyMap = new HashMap<String, String>();
        fillStudySeriesMap(ds, studyMap, seriesMap);
        for (KeySelection ks : keySelection) {
            ks.setStudyUid(studyMap.get(ks.getObjectUid()));
            ks.setSeriesUid(seriesMap.get(ks.getObjectUid()));
        }
    }

    /** Fills the mape of object UID to study/series UIDs */
    public static void fillStudySeriesMap(DicomObject ds, Map<String, String> studyMap, Map<String, String> seriesMap) {
        DicomElement sq = ds.get(Tag.CurrentRequestedProcedureEvidenceSequence);
        int size = sq.countItems();
        for (int i = 0; i < size; i++) {
            DicomObject study = sq.getDicomObject(i);
            String studyUid = study.getString(Tag.StudyInstanceUID);
            DicomElement serSq = study.get(Tag.ReferencedSeriesSequence);
            if (serSq == null) {
                log.warn("Key object has current requested procedured evidence sequence but no reference series...");
                continue;
            }
            int serSize = serSq.countItems();
            for (int j = 0; j < serSize; j++) {
                DicomObject series = serSq.getDicomObject(j);
                String seriesUid = series.getString(Tag.SeriesInstanceUID);
                DicomElement imgSq = series.get(Tag.ReferencedSOPSequence);
                if (imgSq == null) {
                    log.warn("Image sequence in key object isn't found.");
                    continue;
                }
                int imgSize = imgSq.countItems();
                for (int k = 0; k < imgSize; k++) {
                    DicomObject image = imgSq.getDicomObject(k);
                    String sopUid = image.getString(Tag.ReferencedSOPInstanceUID);
                    studyMap.put(sopUid, studyUid);
                    seriesMap.put(sopUid, seriesUid);
                }
            }
        }
    }
    ...
    public Map<QName, String> getOtherAttributes() {
        ...
        if (log.isDebugEnabled())
            log.debug("Getting other attributes=" + ret);
        return ret;
    }
    ...
    /**
     * Clears the macro from this class, and any children of this class. This
     * uses getMacroItems rather than direct access to the variable in order to
     * work with both frame and non-framed images. It still needs extra work for
     * the multi-frame single object, however.
     */

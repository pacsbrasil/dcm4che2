...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="EVN">
        <xsl:param name="eventTypeCode" select="''"/>
        <xsl:param name="eventReasonCode" select="''"/>
        <xsl:param name="operatorId" select="''"/>
        <xsl:text>EVN|</xsl:text><xsl:value-of select="$eventTypeCode"/><xsl:text>|</xsl:text>
        <xsl:value-of select="$messageDateTime"/><xsl:text>||</xsl:text><!-- recorded Date/Time|planned event date/time -->
        <xsl:value-of select="$eventReasonCode"/><xsl:text>|</xsl:text>
        <xsl:value-of select="$operatorId"/><xsl:text>|</xsl:text>
        <xsl:value-of select="$messageDateTime"/><!-- Event occurred Date/Time -->
        <xsl:text>&#xa;</xsl:text>
    </xsl:template>
    
    <xsl:template match="dicom" mode="dcm2PID">
    ...
    <xsl:template match="dicom" mode="dcm2PV1">
    ...
    <xsl:template match="dicom" mode="dcm2ORC">
    ...
    <xsl:template match="dicom" mode="dcm2OBR">
    ...
    <xsl:template match="dicom" mode="dcm2ZDS">

 ...
 * @since 35.03.2005
 */
public class MD5CheckService extends MBeanServiceBase {
    ...
    public void setTaskInterval(String interval) throws Exception {
        ...
        if (isStarted() && oldInterval != taskInterval) {
            ...
            listenerID = scheduler.startScheduler(timerIDCheckMD5,
     ...
     * 
     * @param maxCheckedBefore
     *            The maxCheckedBefore to set.
     */
    ...
    public String check() throws PersistenceException, IOException,
        ...
        for (int j = 0; j < fsdirs.length; j++) {
            files = fsMgt.findFilesForMD5Check(fsdirs[j].getDirectoryPath(),
                    before, limit);
            if (log.isDebugEnabled())
                log.debug("Check MD5 for " + files.length
                        + " files on filesystem " + fsdirs[j]);
            if (files.length > 0) {
                if (buffer == null)
    ...
    private boolean doCheck(FileSystemMgt fsMgt, FileDTO fileDTO, byte[] buffer)
            throws IOException, NoSuchAlgorithmException, PersistenceException {
        if (log.isDebugEnabled())
            log.debug("check md5 for file " + fileDTO);

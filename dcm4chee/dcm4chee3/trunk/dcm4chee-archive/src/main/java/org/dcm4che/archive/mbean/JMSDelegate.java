...
public class JMSDelegate {
    ...
    public JMSDelegate(MBeanServiceBase service) {
        this.server = service.getServer();
        this.logger = service.getLogger();
    }
    
    public JMSDelegate(MBeanServer server, Logger logger){
        this.server = server;        
        this.logger= logger;
    }
    ...
    public void startListening(String name, MessageListener listener,
        ...
        try {
            server.invoke(jmsServiceName,"startListening",
            ...
            logger.fatal("Failed to invoke operation startListening on "
                    + jmsServiceName, e);
        }
    }
    ...
    public void stopListening(String name) throws Exception {
        try {
            server.invoke(jmsServiceName, "stopListening",
                    new Object[] { name },
                    new String[] { String.class.getName() });
        } catch (MBeanException e) {
            throw e.getTargetException();
        } catch (Exception e) {
            logger.fatal("Failed to invoke operation stopListening on "
                    + jmsServiceName, e);
        }
    }
    ...
    public void queue(String name, Serializable obj, int prior,
        ...
        try {
            server.invoke(jmsServiceName, "queue",
            ...
            logger.fatal("Failed to invoke operation queue on " + jmsServiceName, e);
        }
    }
}
 ...
 * @since 35.03.2005
 */
public class FixPatientAttributesService extends MBeanServiceBase {
    ...
    public int repair() throws RemoteException, FinderException,
            CreateException {
        return checkPatientAttributes(true);
    }

    private int checkPatientAttributes(boolean doUpdate)
            throws RemoteException, FinderException {
        FixPatientAttributes checker = newFixPatientAttributes();
        int offset = 0, total = 0;
        int[] fixed;
        do {
            fixed = checker.checkPatientAttributes(offset,
                    limitNumberOfPatientsPerTask, doUpdate);
            total += fixed[0];

 ...
 * 
 */
public class CheckStudyPatientService extends MBeanServiceBase {
    ...
    public void setTaskInterval(String interval) throws Exception {
        ...
        if (isStarted() && oldInterval != taskInterval) {
            ...
            listenerID = scheduler.startScheduler(timerIDCheckStudyPatient,
    ...
    public String getStudyStatus() {
        return studyStatus == null ? "*" : String.valueOf(studyStatus
                .intValue());
    }
    ...
    public void setStudyStatus(String studyStatus) {
        this.studyStatus = "*".equals(studyStatus) ? null : new Integer(
                studyStatus);
    }
    ...
    public String getErrorStudyStatus() {
        return errorStudyStatus == null ? "NONE" : String
                .valueOf(errorStudyStatus.intValue());
    }

    /**
     * @param errorStudyStatus
     *            The errorStudyStatus to set.
     */
    public void setErrorStudyStatus(String errorStudyStatus) {
        this.errorStudyStatus = "NONE".equalsIgnoreCase(errorStudyStatus) ? null
                : new Integer(errorStudyStatus);
    }
    ...
    public String getSuccessStudyStatus() {
        return successStudyStatus == null ? "NONE" : String
                .valueOf(successStudyStatus.intValue());
    }

    /**
     * @param successStudyStatus
     *            The successStudyStatus to set.
     */
    public void setSuccessStudyStatus(String successStudyStatus) {
        this.successStudyStatus = "NONE".equalsIgnoreCase(successStudyStatus) ? null
                : new Integer(successStudyStatus);
    }
    ...
    public String getCheckedStudyStatus() {
        return checkedStudyStatus == null ? "NONE" : String
                .valueOf(checkedStudyStatus.intValue());
    }

    /**
     * @param checkedStudyStatus
     *            The checkedStudyStatus to set.
     */
    public void setCheckedStudyStatus(String checkedStudyStatus) {
        this.checkedStudyStatus = "NONE".equalsIgnoreCase(checkedStudyStatus) ? null
                : new Integer(checkedStudyStatus);
    }
    ...
    public int findStudiesWithPatientCoercion() throws ContentCreateException,
            PersistenceException, PatientMergedException,
            NonUniquePatientException, CircularMergedException {
        return check(false)[1];
    }

    public String check() throws ContentCreateException, PersistenceException,
            PatientMergedException, NonUniquePatientException,
            CircularMergedException {
        int[] ia = check(true);
        if (ia[0] == 0)
            return "Nothing to do!";
        return ia[0] + "studies checked!" + ia[1]
                + " studies moved to new patient(s)!";
    }

    private int[] check(boolean update) throws ContentCreateException,
            PersistenceException, PatientMergedException,
            NonUniquePatientException, CircularMergedException {
        CheckStudyPatient checker = newConsistencyCheck();
        Collection col = checker.findStudiesForTest(studyStatus, sourceAET,
        ...
        for (Iterator iter = col.iterator(); iter.hasNext();) {
            ...
            if (dsa != null) {
                if (dsa[1] != null) {
                    ...
                    log
                            .info("PatientInfo of file and DB differs! A new patient was created and study (pk="
                                    + studyPk + ") moved! new patient:");
                    log.info(ds);
                }
                else {
    ...
    private Dataset[] checkPatInfo(Dataset dsDB, FileDTO fileDTO) {
        log.info("check Patientinfo for " + dsDB.getString(Tags.PatientID)
                + " with file " + fileDTO);
        ...
        try {
            ...
            return comparePatInfo(dsDB, dsFile) ? new Dataset[] { dsDB, null }
                    : new Dataset[] { dsDB, dsFile };
        }
        catch (IOException x) {
    ...
    private boolean comparePatInfo(Dataset dsDB, Dataset dsFile) {
        if (!compareTag(dsDB, dsFile, Tags.PatientID))
            return false;
        if (!compareTag(dsDB, dsFile, Tags.IssuerOfPatientID)) {
            if (!issuer.equals(dsDB.getString(Tags.IssuerOfPatientID)))
                return false;
        }
        if (!compareTag(dsDB, dsFile, Tags.PatientName))
            return false;
        if (!compareTag(dsDB, dsFile, Tags.PatientSex))
            return false;
        if (!compareTag(dsDB, dsFile, Tags.PatientBirthDate))
            return false;
        return true;
    }

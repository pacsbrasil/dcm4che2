 ...
 * @version $Revision: 1.1 $ $Date: 2007/07/09 03:53:28 $
 * @since Dec 7, 2005
 */
public class PPSExceptionMgtService extends MBeanServiceBase implements
        NotificationListener, MessageListener {

    /**
     * Name of the JMS queue to 'serialize' SeriesStored and MPPSReceived
     * Notifictions.
     */
        ...
        try {
            if (notif.getType().equals(MPPSScpService.EVENT_TYPE_MPPS_RECEIVED)) {
                ...
                if (log.isDebugEnabled()) {
                    log.debug("MPPS received. mpps:");
                    log.debug(mppsDS);
                }
                Dataset item = mppsDS
                        .getItem(Tags.PPSDiscontinuationReasonCodeSeq);
                if (item != null
                        && "110514".equals(item.getString(Tags.CodeValue))
                        && "DCM".equals(item
                                .getString(Tags.CodingSchemeDesignator))) {
                    String mppsIUID = mppsDS.getString(Tags.SOPInstanceUID);
                    log
                            .debug("Scheduled: Move discontinued Series (IncorrectWorklistEntry) to trash folder! mppsIuid:"
                                    + mppsIUID);
                    jmsDelegate.queue(queueName, mppsIUID,
                            Message.DEFAULT_PRIORITY, System
                                    .currentTimeMillis()
                                    + getDelay());
                }
            }
            ...
            log
                    .error(
                            "Can not schedule: Move discontinued Series (IncorrectWorklistEntry) to trash folder!",
                            e);
        }
    }
    ...
    public void onMessage(Message message) {
        try {
            ...
            log
                    .info("Move discontinued Series (IncorrectWorklistEntry) to trash folder! mppsIuid:"
                            + mppsIUID);
            ...
            getLogger()
                    .error(
                            "Can not move Series with MPPS 'IncorrectWorklistEntry' to trash folder!",
                            e);
        }
    }
}

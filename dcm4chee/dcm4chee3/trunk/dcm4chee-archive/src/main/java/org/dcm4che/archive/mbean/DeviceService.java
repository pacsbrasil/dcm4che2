 ...
 * @since 17.02.2005
 */
public class DeviceService extends MBeanServiceBase {
    ...
    public String importDeviceMapping(String importURL) {
        ...
        try {
            ...
            t.transform(new StreamSource(importURL), new SAXResult(ds
                    .getSAXHandler2(null)));
    ...
    public String deleteMapping() throws ContentDeleteException,
            PersistenceException {
        return "Status:" + lookupMapper().deleteMapping();
    }

    public String deleteDevice(String device) throws ContentDeleteException,
            PersistenceException {
        return "Status:" + lookupMapper().deleteDevice(device);
    }

    public Collection getDeviceList(String protocol)
            throws ContentCreateException, PersistenceException {
        String[] sa = StringUtils.split(protocol, '^');
        if (sa.length < 3) {
            throw new IllegalArgumentException(
                    "Wrong protocol format! Use <code>^<meaning>^<designator> !");
        }
        Dataset ds = dof.newDataset();

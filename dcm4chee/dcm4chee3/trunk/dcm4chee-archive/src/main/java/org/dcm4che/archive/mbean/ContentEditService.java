 ...
 * @version $Revision: 1.1 $ $Date: 2007/07/08 15:39:26 $
 * @since 17.02.2005
 */
public class ContentEditService extends MBeanServiceBase {
    ...
    public Dataset createPatient(Dataset ds) throws ContentCreateException,
        ...
        if (ds1 != null) {
            throw new ContentCreateException("Patient with ID(pid:" + pid
                    + " issuer:" + issuer + ") already exists!");
        }
    ...
    public List undeletePatient(long privPatPk) throws RemoteException,
            PersistenceException, ContentCreateException {
        if (log.isDebugEnabled())
    ...
    public void deletePatient(long patPk) throws RemoteException,
            ContentCreateException, PersistenceException {
        if (log.isDebugEnabled())
    ...
    public void deleteInstance(long pk) throws RemoteException,
        ...
        lookupPrivateManager().deletePrivateInstance(pk);// dont delete files
        // of instance
        // (needed for purge
        // process)
    }

    public void emptyTrash() throws ContentCreateException,
            PersistenceException {
        if (log.isDebugEnabled())
            log

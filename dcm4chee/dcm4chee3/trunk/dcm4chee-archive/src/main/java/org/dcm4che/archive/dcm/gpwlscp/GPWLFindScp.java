...
import org.dcm4che.archive.dao.jdbc.GPWLQueryCmd;
 ...
 * @version Revision $Date: 2007/07/11 05:10:04 $
 * @since 03.04.2005
 */
    ...
    private class MultiCFindRsp implements MultiDimseRsp {
        ...
        public Dataset next(ActiveAssociation assoc, Dimse rq, Command rspCmd)
            ...
            try {
                ...
                service.getLogger().error("Retrieve DB record failed:", e);
                throw new DcmServiceException(Status.ProcessingFailure, e);
            }
            catch (Exception e) {
                service.getLogger().error("Corrupted DB record:", e);
                throw new DcmServiceException(Status.ProcessingFailure, e);
            }
        }

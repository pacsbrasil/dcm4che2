 ...
 * @since Mar 18, 2007
 */
public class GPWLFeedService2 extends MBeanServiceBase {
    ...
    private void onIAN(Dataset mpps) {
        ...
        try {
            ...
            log.error("Failed to create work items triggered by MPPS from "
                    + aet, e);
            return;
        }
        DcmElement wkitemSeq = wkitems.get(PrivateTags.WorkItemSeq);
        log.info("Creating " + wkitemSeq.countItems()
                + " work item(s) triggered by MPPS from " + aet);
        try {
            createWorkItems(wkitemSeq, contains(pgpAETs, aet), contains(
                    appendAETs, aet));
        }
        catch (Exception e) {
            log.error("Failed to create work item triggered by MPPS from "
                    + aet, e);
        }
    }
    ...
    private synchronized void createWorkItems(DcmElement wkitemSeq,
        ...
        for (int i = 0, n = wkitemSeq.countItems(); i < n; i++) {
            gpwlmgr
                    .addWorklistItem(wkitemSeq.getItem(i), checkPGP,
                            checkAppend);
        }
    }

...
package org.dcm4che.archive.dcm.mwlscp;
 ...
 * @version $Revision: 1.1 $ $Date: 2007/06/14 02:47:12 $
 * @since 31.01.2004
 */
public class MWLFindScpService extends AbstractScpService implements
    ...
    private Dataset getMPPS(String iuid) throws Exception {
        MPPSManager mgr = getMPPSManagerHome();
    ...
    public void handleNotification(Notification notif, Object handback) {
        ...
        MWLManager mgr = getMWLManagerHome();
            ...
            if (spsid != null) {
                try {
                    if (mgr.updateSPSStatus(rpid, spsid, spsStatus)) {
                        log.info("Update MWL item[spsid=" + spsid + ", status="
                                + spsStatus + "]");
                    }
                    else {
    ...
    public boolean checkMWLScuConfig() {
        String proxyAET;
        try {
            proxyAET = (String) server.getAttribute(mwlScuServiceName,
                    "CalledAETitle");
        }
        catch (Exception x) {
            log
                    .warn(
                            "Cant check MWL SCU AET configuration! Continue with assumption that configuration is valid!",
                            x);
            return true;
        }
        if ("LOCAL".equals(proxyAET)) {
            log
                    .warn("Check MWL Proxy Settings: Called AET in MWLScu Service is LOCAL! -> Use local DB access!");
            return false;
        }
        for (int i = 0; i < calledAETs.length; i++) {
            if (proxyAET.equals(calledAETs[i])) {
                log
                        .warn("Check MWL Proxy Settings: Called AET ("
                                + proxyAET
                                + ") in MWLScu is also configured as accepted AET here! -> Disable forwarding MWL C-FIND requests to avoid infinite loop!");
                return false;
            }
        }
        return true;
    ...
    public int findMWLEntries(Dataset rqData, List l, boolean forceLocal)
            throws InstanceNotFoundException, MBeanException,
            ReflectionException {
        return ((Integer) server.invoke(this.mwlScuServiceName,
                forceLocal ? "findMWLEntriesLocal" : "findMWLEntries",
                new Object[] { rqData, l }, new String[] {
                        Dataset.class.getName(), List.class.getName() }))
                .intValue();
    }
}

...
public class DicomSecurityService extends MBeanServiceBase implements
    ...
    private void initSecurityManager() {
        ...
        try {
            ...
            securityManager = (JaasSecurityManager) iniCtx.lookup("java:/jaas/"
                    + securityDomain);
        }
    ...
    public UserIdentityAC negotiate(Association assoc)
        ...
        if (!securityManager.isValid(new SimplePrincipal(userId), passwd,
                subject)) {
            throw new AAssociateRJException(AAssociateRJ.REJECTED_PERMANENT,
                    AAssociateRJ.SERVICE_PROVIDER_ACSE,
                    AAssociateRJ.NO_REASON_GIVEN);
        }
        assoc.putProperty("user", subject);
        return uidRQ != null && uidRQ.isPositiveResponseRequested() ? AssociationFactory
                .getInstance().newUserIdentity()
                : null;
    }

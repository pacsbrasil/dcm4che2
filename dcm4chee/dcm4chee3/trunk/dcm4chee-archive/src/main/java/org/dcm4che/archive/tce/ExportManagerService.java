...
package org.dcm4che.archive.tce;
...
public class ExportManagerService extends AbstractScuService implements
    ...
    private synchronized String nextFilesetID() {
        try {
            server.setAttribute(serviceName, new Attribute(
                    "LastFilesetIDSeqno", new Integer(lastFilesetIDSeqno + 1)));
        }
        catch (Exception e) {
    ...
    private void onSeriesStored(String suid, String code, String designator) {
        try {
            List l = getContentManager().listInstanceInfosByStudyAndSRCode(
                    suid, UIDs.KeyObjectSelectionDocument, code, designator,
                    false);
    ...
    private void delete(Dataset manifest) throws ContentDeleteException {
        ...
        getStorage().deleteInstances(uids, true, false);
    }
    ...
    private void schedule(ExportTFOrder order, long scheduledTime)
        ...
        if (log.isDebugEnabled())
            log.debug("already scheduled:" + scheduledList);
        if (scheduledList.contains(iuid)) {
            log.info("Export TF order " + iuid
                    + " is already scheduled! Ignore this order!");
            return;
        }
        jmsDelegate.queue(queueName, order, Message.DEFAULT_PRIORITY,
                scheduledTime);
        scheduledList.add(iuid);
        copyIUIDs(order.getManifest().get(Tags.IdenticalDocumentsSeq),
                scheduledList);
        while (scheduledList.size() - 100 > 0) {
            log
                    .debug("Scheduled list > 100 -> Remove eldest order from scheduled list:"
                            + scheduledList.remove(0));
        }
    }
    ...
    private void process(ExportTFOrder order) throws Exception {
        ...
        final boolean audit = "YES".equalsIgnoreCase(config
                .getProperty("audit"));
    ...
    private String extractPersonObserverName(Dataset manifest) {
        ...
        for (int i = 0, n = contentSeq.countItems(); i < n; i++) {
            ...
            if (PERSON_OBSERVER_NAME_CODE.equals(conceptName
                    .getString(Tags.CodeValue))
        ...
        try {
            ...
            if (auditLogger.isAuditLogIHEYr4()) {
                ...
                server.invoke(auditLogger.getAuditLoggerName(), "logExport",
                        new Object[] { user, new Patient[] { pat }, null,
                                mediaID, null }, new String[] {
                ...
                msg.addExporterProcess(AuditMessage.getProcessID(),
                        new String[] { callingAET }, AuditMessage
                                .getProcessName(), user == null, AuditMessage
                                .getLocalHostName());
                if (user != null) {
                ...
                for (Iterator iter = sorter.iterateSUIDs(); iter.hasNext();) {
                    ...
                    for (Iterator iter2 = sorter.iterateCUIDs(suid); iter2
                            .hasNext();) {
                        String cuid = (String) iter2.next();
                        SOPClass sopClass = new SOPClass(cuid);
                        sopClass.setNumberOfInstances(sorter.countInstances(
                                suid, cuid));
    ...
    private ActiveAssociation openAssociation(FileInfo[] fileInfos,
        ...
        AE localAE = aeMgt.findByAET(callingAET);
        AE remoteAE = aeMgt.findByAET(dest);
        ...
        Association a = af.newRequestor(tlsConfig.createSocket(localAE,
                remoteAE));
    ...
    public void storeExportSelection(Dataset manifest, int prior)
        ...
        ActiveAssociation aa = openAssociation(dest, manifest
                .getString(Tags.SOPClassUID));
    ...
    private void logBeginTransfering(Association a, Dataset manifest,
            FileInfo[] fileInfos) {
        InstanceSorter sorter = new InstanceSorter();
        sorter.addInstance(manifest.getString(Tags.StudyInstanceUID), manifest
                .getString(Tags.SOPClassUID), manifest
                .getString(Tags.SOPInstanceUID), null);
        DcmElement identicalsq = manifest.get(Tags.IdenticalDocumentsSeq);
        if (identicalsq != null && !identicalsq.isEmpty()) {
            for (int i = 0, n = identicalsq.countItems(); i < n; i++) {
                Dataset otherStudyItem = identicalsq.getItem(i);
                ...
                Dataset otherRefSOPItem = otherSeriesItem
                        .getItem(Tags.RefSOPSeq);
                sorter.addInstance(otherStudyItem
                        .getString(Tags.StudyInstanceUID), otherRefSOPItem
                        .getString(Tags.RefSOPClassUID), otherRefSOPItem
        ...
        String destHost = AuditMessage.hostNameOf(a.getSocket()
                .getInetAddress());

...
import org.dcm4che.archive.common.ActionOrder;
...
import org.dcm4che.archive.config.DeleterThresholds;
 ...
 */
public class FileSystemMgtService extends MBeanServiceBase implements
        MessageListener {
     ...
     *            The AE Title to set.
     */
    public final void setRetrieveAET(String aet) throws PersistenceException,
            IOException {
     ...
     * <p>
     * This option is only effective if a study is deleted on the basis of
     * 'deleteUncommited' policy!
     * 
     * @return
     */
     ...
     * 'deleteUncommited' policy!
     * <p>
     * Only empty patients (No studies, MWL, MPPS, GPWL and GPPPS) will be
     * deleted!
     * 
     ...
     * 
     * @param availability
     *            the file system availability, ONLINE, NEARLINE
    ...
    protected FileSystemDTO initFileSystem(int availability) throws Exception {
        ...
        if (c.length > 0) {
            ...
            c = fsMgt.findFileSystems(retrieveAET, availability,
                    FileSystemStatus.RW);
            if (c.length > 0) {
                fsDTO = c[0];
    ...
    protected void initStorageFileSystem() throws Exception {
        ...
        if (storageFileSystem == null) {
            if (defStorageDir != null && !defStorageDir.equals("NONE")) {
                storageFileSystem = addFileSystem(defStorageDir,
                        ONLINE_STORAGE, retrieveAET, Availability.ONLINE,
                ...
                log
                        .warn("No writeable Storage Directory configured for retrieve AET "
                                + retrieveAET
                                + "- online storage is not available at this moment but should be configured later.");
            }
        }
    ...
    private synchronized boolean switchStorageFileSystem(FileSystemMgt fsMgt,
        ...
        if (storageFileSystem.getPk() != fsDTO.getPk()) {
            log
                    .info("Storage file system has already been switched. Initially was "
                            + fsDTO.toString()
                            + ", now it is "
    ...
    private boolean checkStorageFileSystem(FileSystemDTO fsDTO,
            boolean updateCheckTime) throws IOException {
        ...
        if (updateCheckTime) {
            checkStorageFileSystem = System.currentTimeMillis()
                    + Math.min(freeSpace * checkFreeDiskSpaceMinInterval
                            / minFreeDiskSpace, checkFreeDiskSpaceMaxInterval);
        }
    ...
    public int purgePrivateFiles() {
        ...
        try {
            ...
            try {
                ...
                log.error("Failed to query DB for file system configuration:",
                        e);
                return 0;
            }
    ...
    private long releaseStudy(StudyOnFileSystem studyOnFs,
        ...
        if (release) {
            ...
            size = fsMgt.getStudySize(study.getPk(), studyOnFs.getFileSystem()
                    .getPk());
            this.schedule(new PurgeStudyOrder(study.getPk(), studyOnFs
                    .getFileSystem().getPk(), deleteUncommited,
                    deleteEmptyPatient), System.currentTimeMillis());
            ...
            if (log.isDebugEnabled())
                log.debug("Study [" + study.getStudyIuid()
                        + "] doesn't fulfill any deleter rule!");
        }
        return size;
    }
    ...
    /**
     * Show all file systems in unsorted format given the availability and
     * status.
     * 
     * @param availability
     *            The availability of file system, e.g., ONLINE, NEARLINE. If
     *            it's negative, ignored.
     * @param status
     *            The status of file system, e.g., RW+, RW, RO. If it's
     *            negative, ignored.
     * @return
     * @throws
     * @throws PersistenceException
        ...
        if (availability >= -1 || status >= -1) {
            ...
            for (int i = 0; i < dtos.length; i++) {
                if ((availability < -1 || availability >= -1
                        && dtos[i].getAvailability() == availability)
                        && (status < -1 || status >= -1
                                && dtos[i].getStatus() == status))
                    dtoa.add(dtos[i]);
            }
            if (dtoa.size() == 0)
                return "NONE";
            dtos = (FileSystemDTO[]) dtoa
                    .toArray(new FileSystemDTO[dtoa.size()]);
        }

        return toString(dtos);
    }

    public String showAllFileSystems() throws PersistenceException {
        FileSystemDTO[] dtos = listAllFileSystems();
    ...
    private FileSystemDTO addFileSystem(String dirPath, String groupId,
            String retrAET, int availability, int status, String userInfo)
            throws ContentCreateException {
        FileSystemDTO dto = new FileSystemDTO();
        dto.setDirectoryPath(dirPath);
        dto.setGroupId(groupId);
        dto.setRetrieveAET(retrAET);
    ...
    public int updateAETitle(String prevAET, String newAET) throws Exception {
        int count = 0;
        for (int i = 0; i < otherServiceNames.length; i++) {
        ...
        server.invoke(aeServiceName, "updateAETitle", new Object[] { prevAET,
                newAET }, new String[] { String.class.getName(),
                String.class.getName() });
        return count;
    }

    private boolean updateAETitle(ObjectName name, String attr, String prevAET,
            String newAET) throws Exception {
        try {
            ...
            if (modified) {
                server.setAttribute(name, new Attribute(attr, StringUtils
                        .toString(aets, '\\')));
    ...
    private boolean updateFileSystemRetrieveAET(String dirPath,
            String retrieveAET, FileSystemMgt mgt) throws PersistenceException {
    ...
    public String showOnlineFileSystems() throws PersistenceException {
    ...
    public String showNearlineFileSystems() throws PersistenceException {
    ...
    public int deleteWholeStudy(String studyIUID, boolean deletePatientIfEmpty)
        ...
        File file;
        for (int i = 0; i < fileDTOs.length; i++) {
            file = FileUtils.toFile(fileDTOs[i].getDirectoryPath(), fileDTOs[i]
                    .getFilePath());
            delete(file);
        }

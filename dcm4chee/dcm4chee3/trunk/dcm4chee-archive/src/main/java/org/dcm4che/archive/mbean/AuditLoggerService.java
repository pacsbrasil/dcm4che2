 ...
 */
public class AuditLoggerService extends MBeanServiceBase {
    ...
    public String getSupressLogForAETs() {
        if (supressLogForAETs.isEmpty())
            return "NONE";
        StringBuilder sb = new StringBuilder((String) supressLogForAETs.get(0));
    ...
    public void setSyslogHost(String newSyslogHost) throws Exception {
        ...
        logActorConfig(actorName, "SyslogHost", getSyslogHost(), trimed,
    ...
    public void setLogPatientRecord(boolean enable) {
        logActorConfig(actorName, "LogPatientRecord", new Boolean(
                isLogPatientRecord()), new Boolean(enable),
                AuditLogger.SECURITY);
        logger.setLogPatientRecord(enable);
    }

    public void setLogProcedureRecord(boolean enable) {
        logActorConfig(actorName, "LogProcedureRecord", new Boolean(
                isLogProcedureRecord()), new Boolean(enable),
                AuditLogger.SECURITY);
    ...
    public void setFacility(String newFacility) {
        ...
        logActorConfig(actorName, "Facility", oldFacility, trimed,
                AuditLogger.SECURITY);
    }
    ...
    public void setStrictIHEYr4(boolean enable) {
        logActorConfig(actorName, "StrictIHEYr4",
                new Boolean(isStrictIHEYr4()), new Boolean(enable),
                AuditLogger.SECURITY);
    ...
    public void logActorConfig(String actor, String propName, Object oldVal,
            Object newVal, String type) {
        if (!newVal.equals(oldVal)) {
            logActorConfig(actor + ": " + propName + " changed from " + oldVal
                    + " to " + newVal, type);
        }
    }
    ...
    public void logStudyDeleted(String patid, String patname, String suid,
        ...
        if (isStarted()) {
            ...
            InstancesAction action = alf.newInstancesAction("Delete", suid,
                    patient);
            if (numInsts != null)
    ...
    public void logProcedureRecord(String action, String patid, String patname,
        ...
        if (isStarted()) {
            Patient patient = alf.newPatient(patid, patname);
            logger.logProcedureRecord(action, patient, placerOrderNo,
                    fillerOrderNo, suid, accNo, getCurrentUser(), desc);
        }
    }

    public void logInstancesStored(RemoteNode node, InstancesAction action) {
        if (isStarted() && !supressLogForAETs.contains(node.getAET())) {
    ...
    public void logSecurityAlert(String alertType, Socket socket, String aet,
            String description) {
        if (isStarted() && !supressLogForAETs.contains(aet)) {
    ...
    public void logExport(String patid, String patname, String mediaType,
            Set suids, String ip, String host, String aet) {
        Patient patient = alf.newPatient(patid, patname);
        if (suids != null) {
        ...
        descr.setDestination(alf.newRemotePrinter(alf.newRemoteNode(ip, host,
                aet)));

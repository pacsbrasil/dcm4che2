 ...
 * @since Dec 12, 2005
 */
public class StudyStatusService extends MBeanServiceBase implements
    ...
    public void handleNotification(Notification notif, Object handback) {
        ...
        if (status == null) {
            if (log.isDebugEnabled())
                log
                        .debug("No Status Update Rule for Performed Work Item Code: "
                                + key);
            return;
        }
        ...
        if (suid == null) {
            log
                    .debug("Missing Study Instance UID in Ref.Request Item - No Status Update");
            return;
        }
        ...
        try {
            ...
            log.error(
                    "Failed to update Status ID of Study[iuid=" + suid + "]:",
                    e);
        }
    }
}

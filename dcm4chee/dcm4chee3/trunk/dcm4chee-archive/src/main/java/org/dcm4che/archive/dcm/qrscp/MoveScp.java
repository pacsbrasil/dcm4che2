...
public class MoveScp extends DcmServiceBase implements AssociationListener {
    ...
    public void c_move(ActiveAssociation assoc, Dimse rq) throws IOException {
        ...
        try {
            ...
            try {
                ...
                InetAddress host = dest.equals(assoc.getAssociation()
                        .getCallingAET()) ? assoc.getAssociation().getSocket()
                        .getInetAddress() : null;
                ...
                service.getLogger().error("Query DB failed:", e);
                throw new DcmServiceException(Status.UnableToProcess, e);
            }
            catch (Throwable e) {
                service.getLogger().error("Unexpected exception:", e);
                throw new DcmServiceException(Status.UnableToProcess, e);
            }
        }
    ...
    protected MoveTask createMoveTask(QueryRetrieveScpService service,
            ...
            Dataset moveRqData, FileInfo[][] fileInfo, AE aeData,
            String moveDest) throws DcmServiceException {
        return new MoveTask(service, moveAssoc, movePcid, moveRqCmd,
                moveRqData, fileInfo, aeData, moveDest);
    }

    private void checkMoveRQ(Association assoc, int pcid, Command rqCmd,
            Dataset rqData) throws DcmServiceException {

        checkAttribute(rqCmd, Tags.MoveDestination, Status.UnableToProcess,
                "Missing Move Destination");
        checkAttribute(rqData, Tags.QueryRetrieveLevel,
        ...
        final String asid = assoc.getProposedPresContext(pcid)
        ...
        if ("PATIENT".equals(level)) {
            ...
            checkAttribute(rqData, Tags.PatientID,
                    Status.IdentifierDoesNotMatchSOPClass, "Missing Patient ID");
        }
        else if ("STUDY".equals(level)) {
            checkAttribute(rqData, Tags.StudyInstanceUID,
                    Status.IdentifierDoesNotMatchSOPClass,
                    "Missing Study Instance UID");

        }
        else if ("SERIES".equals(level)) {
            if (UIDs.PatientStudyOnlyQueryRetrieveInformationModelMOVE
                    .equals(asid)) {
                throw new DcmServiceException(
                        Status.IdentifierDoesNotMatchSOPClass,
                        "Cannot use Query Retrieve Level SERIES with Patient Study Only IM");
            }
            checkAttribute(rqData, Tags.SeriesInstanceUID,
                    Status.IdentifierDoesNotMatchSOPClass,
            ...
            checkAttribute(rqData, Tags.SOPInstanceUID,
                    Status.IdentifierDoesNotMatchSOPClass,

 ...
 * @since Aug 27, 2007
 */
public class DicomSecurityDelegate {

    private final ServiceMBeanSupport service;

    private ObjectName dicomSecurityServiceName;

    public DicomSecurityDelegate(final ServiceMBeanSupport service) {
        this.service = service;
    }

    public final ObjectName getDicomSecurityServiceName() {
        return dicomSecurityServiceName;
    }

    public final void setDicomSecurityServiceName(ObjectName serviceName) {
        this.dicomSecurityServiceName = serviceName;
    }

    public UserIdentityNegotiator userIdentityNegotiator() {
        try {
            return (UserIdentityNegotiator) service.getServer().invoke(
                    dicomSecurityServiceName, "userIdentityNegotiator", null,
                    null);
        }
        catch (Exception e) {

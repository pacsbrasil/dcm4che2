 ...
 * @version $Revision: 1.3 $ $Date: 2007/07/11 05:10:04 $
 * @since Feb 1, 2006
 */
class VMFBuilder {
    private static final int[] SHARED_FG = { Tags.SharedFunctionalGroupsSeq, };

    private static final int[] COMMON = { Tags.SOPClassUID, Tags.Rows,
            Tags.Columns, Tags.BitsAllocated, Tags.BitsStored, Tags.HighBit };
    ...
    public VMFBuilder(QueryRetrieveScpService service, Dataset firstFrame,
        ...
        this.result.putUI(Tags.SOPInstanceUID, UIDGenerator.getInstance()
                .createUID());
        this.sharedFGs = result.putSQ(Tags.SharedFunctionalGroupsSeq)
                .addNewItem();
    ...
    private void addFrameInternal(Dataset frame) {
        ...
        refImg.putUI(Tags.RefSOPInstanceUID, frame

...
import org.dcm4che.archive.dao.jdbc.QueryOldARRCmd;
import org.dcm4che.archive.dao.jdbc.QueryOldARRCmd.Record;
...
public class MigrateOldAuditRecordRepositoryService extends ServiceMBeanSupport {
    ...
    public final void setEmissionInterval(int emissionInterval) {
        if (emissionInterval < 0) {
            throw new IllegalArgumentException("emissionInterval: "
                    + emissionInterval);
        }
    ...
    public String emitAuditRecords(int num) throws Exception {
        ...
        while (count < num) {
            int emitted = emitBlockOfAuditRecords(Math.min(num - count,
                    recordsByPass));
            if (emitted == 0) {
                break;
            }
    ...
    private int emitBlockOfAuditRecords(int limit) throws Exception {
        ...
        try {
            ...
            if (sent > 0) {
                ...
                server.setAttribute(serviceName, new Attribute("LastEmittedPk",
                        new Long(lastEmittedPk)));
            }
        }
    }
}

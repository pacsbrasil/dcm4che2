 ...
 * @since 04.01.2006
 */
public class KeyObjectService extends MBeanServiceBase {
    ...
    /**
     * @param compositSopClassUIDs
     *            The compositSopClassUIDs to set.
     */
    ...
    /**
     * @param waveformSopClassUIDs
     *            The waveformSopClassUIDs to set.
     */
    ...
    private String getCUIDs(Map map) {
        if (map == null || map.isEmpty())
            return "";
        StringBuilder sb = new StringBuilder(map.size() << 5);// StringBuilder
        // initial size:
        // nrOfUIDs x 32
    ...
    private void setDefaultWaveformSopCuids() {
        ...
        waveformSopClassUIDs.put("TwelveLeadECGWaveformStorage",
                UIDs.TwelveLeadECGWaveformStorage);
        waveformSopClassUIDs.put("GeneralECGWaveformStorage",
                UIDs.GeneralECGWaveformStorage);
        waveformSopClassUIDs.put("AmbulatoryECGWaveformStorage",
                UIDs.AmbulatoryECGWaveformStorage);
        waveformSopClassUIDs.put("HemodynamicWaveformStorage",
                UIDs.HemodynamicWaveformStorage);
        waveformSopClassUIDs.put("CardiacElectrophysiologyWaveformStorage",
    ...
    private void setDefaultCompositeSopCuids() {
        ...
        compositSopClassUIDs.put("KeyObjectSelectionDocument",
                UIDs.KeyObjectSelectionDocument);
    }

    public Dataset getKeyObject(String iuids) throws PersistenceException {
     ...
     * <p>
     * The content sequence will contain all items from
     * <code>contentItems</code> and references to all instances given in
     * iuids.
     * <p>
     * If instances from differet studies (but same patient) are given, the
     * dataset will contain the Identical Document sequence with references to
     * copies of this Key Object in the approbiate study. (new generated Series
     * and SOP Instance UID). This Sequence can be used to create the copies of
     * the Key object.
     * <p>
     * If instances from different patients are given, an
     * IllegalArgumentException will be thrown.
     * 
     * @param iuids
     *            List of instances referenced by this Key Object.
     * @param rootInfo
     *            Dataset with Attributes for root node. (e.g. Concept Name code
     *            sequence for title)
     * @param contentItems
     *            List of Datasets. each dataset is an item of the Content
     *            Sequence.
     * 
     ...
     */
    public Dataset getKeyObject(Collection iuids, Dataset rootInfo,
            Collection contentItems) throws PersistenceException {
        Collection col = lookupContentManager().getSOPInstanceRefMacros(iuids);
        Dataset ds;
        if (!col.isEmpty()) {
            String studyIUID = ((Dataset) col.iterator().next())
    ...
    private Dataset newKeyObject(String studyIUID, Dataset rootInfo) {
        ...
        ds.putUI(Tags.StudyInstanceUID, studyIUID != null ? studyIUID
                : uidGenerator.createUID());
     ...
     * @param col
     */
    private void addContentSequence(Dataset ds, Collection conceptNames,
            Collection refSopInstances) {
        ...
        for (Iterator iter = refSopInstances.iterator(); iter.hasNext();) {
            ...
            for (int i = 0, len = refSerSq.countItems(); i < len; i++) {
                ...
                for (int j = 0, n = refSopSq.countItems(); j < n; j++) {
                    ...
                    item.putCS(Tags.ValueType, getValueType(refSopItem
                            .getString(Tags.RefSOPClassUID)));
                    Dataset sopDS = item.putSQ(Tags.RefSOPSeq).addNewItem();
                    sopDS.putUI(Tags.RefSOPClassUID, refSopItem
                            .getString(Tags.RefSOPClassUID));
                    sopDS.putUI(Tags.RefSOPInstanceUID, refSopItem
     ...
     * @throws PersistenceException
     */
    private void addReqProcEvidenceSequence(Dataset ds, Collection col)
            throws PersistenceException {
        ...
        if (iter.hasNext()) {
            ...
            String patID1;
            for (; iter.hasNext();) {
                // loop only if this key object selection contains instances in
                // different studies
                studyDS = (Dataset) iter.next();
                patID1 = lookupContentManager().getPatientForStudy(
                        studyDS.getString(Tags.StudyInstanceUID)).getString(
                        Tags.PatientID);
                if (!patID.equals(patID1)) {
                    throw new IllegalArgumentException(
                            "Instances of different patients are not allowed!");
                }

                // add to ReqProcEvidenceSequence
                sq.addItem(studyDS);
                // add copy reference to identical document sequence
    ...
    private void addSOPInstanceRef(DcmElement sq, Dataset studyDS) {
        ...
        ds.putUI(Tags.StudyInstanceUID, studyDS
                .getString(Tags.StudyInstanceUID));
        DcmElement serSq = ds.putSQ(Tags.RefSeriesSeq);
        Dataset serDS = serSq.addNewItem();
        serDS.putUI(Tags.SeriesInstanceUID, UIDGenerator.getInstance()
                .createUID());
        DcmElement refSopSq = serDS.putSQ(Tags.RefSOPSeq);
        Dataset instDS = refSopSq.addNewItem();
        instDS.putUI(Tags.RefSOPInstanceUID, UIDGenerator.getInstance()
                .createUID());
     ...
     * @throws PersistenceException
     */
    private void addPatInfo(Dataset ds, String studyIUID)
            throws PersistenceException {

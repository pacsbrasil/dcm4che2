...
package org.dcm4che.archive.dcm.gpwlscp;
...
class PPSScp extends DcmServiceBase {
    ...
    private static final int[] TYPE1_NCREATE_ATTR = { Tags.PPSID,
            Tags.PPSStartDate, Tags.PPSStartTime, Tags.GPPPSStatus };

    private static final int[] TYPE1_REF_GPSPS_ATTR = { Tags.RefSOPClassUID,
            Tags.RefSOPInstanceUID, Tags.RefGPSPSTransactionUID };
    ...
    private static final int[] TYPE1_CODEITEM_ATTR = { Tags.CodeValue,
            Tags.CodingSchemeDesignator, Tags.CodeMeaning };

    private static final int[] ONLY_NCREATE_ATTR = { Tags.RefRequestSeq,
            Tags.RefGPSPSSeq, Tags.PatientName, Tags.PatientID,
            Tags.PatientBirthDate, Tags.PatientSex,
            Tags.ActualHumanPerformersSeq, Tags.PPSID,
            Tags.PerformedStationNameCodeSeq,
            Tags.PerformedStationClassCodeSeq,
            Tags.PerformedStationGeographicLocationCodeSeq,
            Tags.PerformedProcessingApplicationsCodeSeq,
    ...
    private void checkCreateAttributs(Dataset gppps) throws DcmServiceException {
                ...
                for (int j = 0; j < TYPE1_REF_GPSPS_ATTR.length; ++j) {
                    if (!refGPSPS.containsValue(TYPE1_REF_GPSPS_ATTR[j]))
                        throw new DcmServiceException(
                                ...
                                "Missing Type 1 Attribute "
                                        + Tags
                                                .toString(TYPE1_REF_GPSPS_ATTR[j])
        ...
        throw new DcmServiceException(Status.InvalidAttributeValue,
                "Invalid General Purpose Performed Procedure Step Status: "
                        + status);
    }
    ...
    private void checkSetAttributs(Dataset gppps) throws DcmServiceException {
        ...
        try {
            if (status == null
                    || PPSStatus.toInt(status) == PPSStatus.IN_PROGRESS) {
                return;
            }
        }
        catch (IllegalArgumentException e) {

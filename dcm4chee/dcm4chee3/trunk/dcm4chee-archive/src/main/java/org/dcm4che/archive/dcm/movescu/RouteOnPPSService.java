...
package org.dcm4che.archive.dcm.movescu;
 ...
 * @version $Revision: 1.4 $ $Date: 2007/07/19 06:22:57 $
 * @since Dec 19, 2006
 */
public class RouteOnPPSService extends MBeanServiceBase implements
    ...
    public void handleNotification(Notification notif, Object handback) {
        ...
        if (suid == null) {
            log
                    .warn("Missing Study Instance UID in Ref.Request Item - Routing abandoned");
            return;
        }

        String aet;
        for (StringTokenizer st = new StringTokenizer(aets, ","); st
                .hasMoreTokens();) {
            aet = st.nextToken();
    ...
    private void scheduleMove(String retrieveAET, String destAET, String patId,
            String studyIUID) {
        try {
            server.invoke(moveScuServiceName, "scheduleMove", new Object[] {
                    retrieveAET, destAET, new Integer(this.routingPriority),
                    patId, studyIUID, null, null, new Long(-1l) },
    ...
    public void testRouting(String code, String studyUID) {
        ...
        codeItem.putSH(Tags.CodeValue, pos == -1 ? code : code
                .substring(0, pos));
        if (pos != -1)
            codeItem.putSH(Tags.CodingSchemeDesignator, code.substring(++pos));
        Dataset item = ds.putSQ(Tags.RefRequestSeq).addNewItem();
        item.putUI(Tags.StudyInstanceUID, studyUID);
        Notification notif = new Notification("testRouting", this, System
                .currentTimeMillis());

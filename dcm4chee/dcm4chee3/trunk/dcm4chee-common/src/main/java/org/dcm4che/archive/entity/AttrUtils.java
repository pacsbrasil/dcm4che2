...
public class AttrUtils {
    ...
    public static boolean coerceAttributes(Dataset oldAttrs, Dataset newAttrs,
        ...
        for (Iterator it = oldAttrs.iterator(); it.hasNext();) {
            ...
            if (!equals(el, newAttrs.getSpecificCharacterSet(), refEl, oldAttrs
                    .getSpecificCharacterSet(), coercedElements, log)) {
                if (filter != null && filter.isCoercionForbidden(tag)) {
                    throw new DcmServiceException(FORBIDDEN_ATTRIBUTE_COERCION,
                            "Storage would require forbidden Coercion of " + el
                                    + " to " + refEl);
                }
    ...
    private static boolean equals(DcmElement el, SpecificCharacterSet cs,
            ...
            Dataset coercedElements, Logger log) throws DcmServiceException {
        ...
        if (vr == VRs.SQ) {
            ...
            try {
                if (vr == VRs.PN ? !Arrays.equals(refEl.getPersonNames(refCS),
                        el.getPersonNames(cs)) : !Arrays.equals(refEl
                        .getStrings(refCS), el.getStrings(cs))) {
                    return false;
                }
            }
        ...
        for (Iterator it = newAttrs.iterator(); it.hasNext();) {
            ...
            if (dsEl.hasItems()) {
                ...
                if (refEl == null || refEl.isEmpty()) {
                    log
                            .info("Update stored objects with additional element/value from new received object- "
                                    + dsEl);
                    ...
                    for (int i = 0; i < numItems; i++)
                        if (mergeAttributes(refEl.getItem(), dsEl.getItem(),
                                log))
                            updateEntity = true;
                }
            }

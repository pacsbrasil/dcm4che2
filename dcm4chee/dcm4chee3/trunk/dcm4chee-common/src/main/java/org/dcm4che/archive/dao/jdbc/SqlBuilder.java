...
class SqlBuilder {
    ...
    public Match addFieldValueMatch(String alias1, String field1,
            boolean type2, String alias2, String field2) {
        return addMatch(new Match.FieldValue(alias1, field1, type2, alias2,
                field2));
    }
    ...
    public Match addListOfStringMatch(String alias, String field,
            boolean type2, String[] vals) {
    ...
    public void addPNMatch(String[] nameFields, boolean type2, String val) {
        ...
        if (pn != null) {
            addWildCardMatch(null, nameFields[0], type2, toUpperCase(pn
                    .toComponentGroupMatch()));
        }
        PersonName ipn = pn.getIdeographic();
        if (ipn != null) {
            addWildCardMatch(null, nameFields[1], type2, ipn
                    .toComponentGroupMatch());
        }
        PersonName ppn = pn.getPhonetic();
        if (ppn != null) {
            addWildCardMatch(null, nameFields[2], type2, ppn
                    .toComponentGroupMatch());
        }
    }
    ...
    public Match getRangeMatch(String alias, String field, boolean type2,
            Date[] range) {
        return new Match.Range(alias, field, type2, range,
                getDatabase() == JdbcProperties.ORACLE ? ORA_DATE_FORMAT
                        : DATE_FORMAT);
    }
    ...
    public Match addCallingAETsNestedMatch(boolean privateTables,
            String[] callingAETs) {
        return addMatch(new Match.CallingAETsNestedMatch(privateTables,
                callingAETs));
    }

    public Match addQueryPermissionNestedMatch(String[] roles) {
        return addMatch(new Match.QueryPermissionNestedMatch(roles));
    }
    ...
    public Match addUncorrelatedSubquery(SqlBuilder subQuery, String field,
            String alias) {
        ...
        if (limit > 0 || offset > 0) {
            ...
            case JdbcProperties.MSSQL:
                if (!orderby.isEmpty()) {
                    ...
                    sb.append(" FROM ( SELECT TOP ").append(limit + offset)
                            .append(' ');
                    appendTo(sb, selectAsC1C2CN());
                    sb.append(',');
    ...
    private String[] selectOrderByAsSort() {
        ...
        for (int i = 0; i < retval.length; i++) {
            ...
            retval[i] = s.substring(0, s.lastIndexOf(' ')) + " AS sort"
                    + (i + 1);
        }
        return retval;
    }
    ...
    private void appendLeftJoinToWhere(StringBuffer sb) {
        if (leftJoin == null || getDatabase() != JdbcProperties.ORACLE)
            return;
        for (int i = 0, n = leftJoin.length / 4; i < n; ++i) {
            final int i4 = 4 * i;

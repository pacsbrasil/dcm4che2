...
public class RetrieveStudyDatesCmd extends BaseReadCmd {
    ...
    // be aware of order the fields: dont mix up different types (updated,
    // created)
    private static final String[] SELECT_ATTRIBUTE = { "Patient.updatedTime",
            "Study.updatedTime", "Series.updatedTime", "Instance.updatedTime",
            "Patient.createdTime", "Study.createdTime", "Series.createdTime",
    ...
    public static RetrieveStudyDatesCmd create(Dataset keys)
        ...
        if ("PATIENT".equals(qrLevel)) {
            return new RetrieveStudyDatesCmd(new PatientSql(keys, true)
                    .getSql());
        }
     ...
     * @param endIdx
     * @param eldest
     *                If true use boundary 'eldest', otherwise 'most recent'
     * @return
     * @throws SQLException
     */
    private Date getBoundary(int startIdx, int endIdx, boolean eldest)
            throws SQLException {
        try {
            rs.beforeFirst();
            if (!next()) {

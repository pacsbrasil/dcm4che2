 ...
 * @version $Id: FileSystemUtils.java,v 1.1 2007/06/12 21:03:20 asd Exp $
...
public class FileSystemUtils {
    ...
    static {
        try {
            jdk6getFreeSpace = File.class.getMethod("getFreeSpace",
                    (Class[]) null);
        }
        catch (Exception e) {
    ...
    public static long freeSpace(String path) throws IOException {
        if (jdk6getFreeSpace != null) {
            try {
                Long l = (Long) jdk6getFreeSpace.invoke(new File(path),
                        (Object[]) null);
                return l.longValue();
            }
            catch (Exception e) {
    ...
    long freeSpaceWindows(String path) throws IOException {
        ...
        outerLoop: while (i > 0) {
            ...
            if (line.length() > 0) {
                ...
                // find the first pair of non-numeric character, and everything
                // between
                ...
                innerLoop2: while (j >= 0) {// Loop until found two adjoining
                                            // non digit characters.
                    char c = line.charAt(j);
                    if (!Character.isDigit(c)
                            && (j == 0 || !Character
                                    .isDigit(line.charAt(j - 1)))) {
                        break innerLoop2;
                    }
                    j--;
                }
    ...
    public static long parseDF(String path, String line1, String line2,
    ...
    long parseUnixDF(String path, String line1, String line2, String line3)
        ...
        if (line2 == null) {
            // unknown problem, throw exception
            throw new IOException("'" + dfCommand + " " + dfCommandOption
        ...
        if (blocksizeEnd > 0) {
            ...
            while (blocksizePos > 0
                    && Character.isDigit(line1.charAt(blocksizePos - 1))) {
                --blocksizePos;
            }
            if (blocksizePos < blocksizeEnd) {
                blocksize = factor
                        * Long.parseLong(line1.substring(blocksizePos,
                                blocksizeEnd));
            }
        }

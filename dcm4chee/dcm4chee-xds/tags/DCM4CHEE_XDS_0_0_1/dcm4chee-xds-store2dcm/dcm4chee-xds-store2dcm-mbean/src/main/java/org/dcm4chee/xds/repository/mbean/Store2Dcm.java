...
public class Store2Dcm {
    ...
    public Store2Dcm(File xmlFile, XDSDocumentWriter docWriter, 
            Source metadata, File xslFile) throws ParserConfigurationException, SAXException, IOException, TransformerException {
    ...
    public Store2Dcm(File xmlFile, XDSDocumentWriter docWriter, 
            Source metadata, Source xslSource) throws ParserConfigurationException, SAXException, IOException, TransformerException {
        this.xmlFile = xmlFile;
        this.docWriter = docWriter;
        this.metadata = metadata;
    ...
    private void addMissingAttributes(DicomObject attrs) {
        ...
        if ( attrs.get(Tag.SOPClassUID) == null)
            attrs.putString(Tag.SOPClassUID, VR.UI, getCuidForMime(mime));
        if ( attrs.get(Tag.SpecificCharacterSet) == null)
            attrs.putString(Tag.SpecificCharacterSet, VR.CS, charset);
        if ( attrs.get(Tag.Modality) == null)
            attrs.putString(Tag.Modality, VR.CS, "XDS");
    ...
    private String getCuidForMime(String mime) {
		...
		if ( cuid == null )
			cuid = (String)mime2CuidMap.get(KEY_DEFAULT_CUID);

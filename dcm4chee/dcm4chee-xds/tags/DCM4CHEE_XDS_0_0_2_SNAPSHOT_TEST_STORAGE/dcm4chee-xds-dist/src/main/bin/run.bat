...
rem $Id: run.bat 6238 2008-04-28 14:05:01Z javawilli $
...
rem Sample https protocol settings
rem Uncomment to disable SSLv2Hello when SSL handshake failed with 'javax.net.ssl.SSLHandshakeException: SSLv2Hello is disabled'
rem set JAVA_OPTS=%JAVA_OPTS% -Dhttps.protocols=SSLv3,TLSv1

rem Setup JBoss HTTP proxy server configuration
rem set JAVA_OPTS=%JAVA_OPTS% -Dhttp.proxyHost=<proxy server> -Dhttp.proxyPort=<proxy port> -DproxySet=true

...
>
  ...
  <!-- ExtrinsicObject for one Document -->
  ...
  <xsl:template match="rim:Slot[@name='creationTime']" >
    <xsl:variable name="datetime" select="rim:ValueList/rim:Value"/>
    <xsl:variable name="date" select="substring($datetime,1,8)"/>
	<!-- Study date -->
  ...
  <xsl:template match="rim:Value" mode="patInfo">
	    ...
	    <attr tag="00100010" vr="PN" ><xsl:value-of select="$pid_value" /></attr>
    	    ...
    	    <!--Patients Birthdate-->
	    <attr tag="00100030" vr="DA" ><xsl:value-of select="substring($pid_value,1,8)" /></attr>
            </xsl:when>
  ...
  <!--  put XDSDocumentEntry.patientId (Affinity Domain) to other patient sequence -->

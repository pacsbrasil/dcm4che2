...
public class DocumentStorageListenerTest extends DocStoreTestBase {
    ...
    /*    DocumentStorage st = docStore.selectDocStorageFromPoolOrDomain(null);
        List l = listener.events[TestDocumentStorageListener.AVAILABILITY];
        assertEquals("Wrong number of recorded AVAILABILITY events!", 0, l.size());
        toggleAvailability(st);
        assertEquals("Wrong number of recorded AVAILABILITY events!", 1, l.size());
        toggleAvailability(st);
        assertEquals("Wrong number of recorded AVAILABILITY events!", 2, l.size());
    ...
    private final class TestDocumentStorageListener implements

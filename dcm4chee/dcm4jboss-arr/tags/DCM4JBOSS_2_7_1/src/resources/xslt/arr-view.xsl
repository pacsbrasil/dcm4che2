...
<td width="10"></td>
	...
	<xsl:template match="Import">
		<span class="event">Import</span><br/><br/>
		<xsl:apply-templates select="MediaDescriptionType"/>
		<xsl:apply-templates select="User"/>
	</xsl:template>
	<xsl:template match="MediaDescriptionType">
		<b>MediaDescriptionType:</b><br/><div class="level"><xsl:call-template name="MediaDescriptionType"/></div>
	</xsl:template>
	<xsl:template match="User">
		<b>User:</b><br/><div class="level"><xsl:call-template name="UserType"/></div>
	</xsl:template>
	<!-- InstancesStored -->
	<xsl:template match="InstancesStored">
		<span class="event">InstancesStored</span><br/><br/>
		<xsl:apply-templates select="RemoteNode"/>
		<xsl:apply-templates select="InstanceActionDescription"/>
	</xsl:template>
	<xsl:template match="RemoteNode">
		<b>RemoteNode:</b><br/><div class="level"><xsl:call-template name="RemoteNodeType"/></div>
	</xsl:template>
	<xsl:template match="InstanceActionDescription">
		<b>InstanceActionDescription:</b><br/><div class="level"><xsl:call-template name="InstancesActionType"/></div>
	</xsl:template>
	<!-- ProcedureRecord -->
	<xsl:template match="ProcedureRecord">
		<span class="event">ProcedureRecord</span><br/><br/>
		<xsl:apply-templates select="ObjectAction"/>
		<xsl:if test="PlacerOrderNumber"><b>PlacerOrderNum: </b><span class="num"><xsl:value-of select="PlacerOrderNumber"/></span><br/></xsl:if>
		<xsl:if test="FillerOrderNumber"><b>FillerOrderNum: </b><span class="num"><xsl:value-of select="FillerOrderNumber"/></span><br/></xsl:if>
	...
	<xsl:template match="ActorStartStop">
		<span class="event">ActorStartStop</span><br/><br/>
		<xsl:if test="ActorName"><b>ActorName: </b><xsl:value-of select="ActorName"/><br/></xsl:if>
	...
	<xsl:template match="Patient">
		<b>Patient:</b><br/><div class="level"><xsl:call-template name="PatientType"/></div>
	</xsl:template>
	<!-- OrderRecord -->
	<xsl:template match="OrderRecord">
		<span class="event">OrderRecord</span><br/><br/>
		<xsl:apply-templates select="ObjectAction"/>
		<xsl:apply-templates select="Patient"/>
		<xsl:apply-templates select="User"/>
	</xsl:template>
	...
	<xsl:template match="BeginStoringInstances">
		<span class="event">BeginStoringInstances</span><br/><br/>
		<xsl:call-template name="BeginStoringInstancesType"/>
		<xsl:apply-templates select="Rnode"/>
		<xsl:apply-templates select="InstanceActionDescription"/>
	</xsl:template>
	<xsl:template match="Rnode">
		<b>Rnode:</b><br/><div class="level"><xsl:call-template name="RemoteNodeType"/></div>
	</xsl:template>
	<xsl:template match="RNode">
		<b>RNode:</b><br/><div class="level"><xsl:call-template name="RemoteNodeType"/></div>
	</xsl:template>
	...
	<xsl:template match="InstancesSent">
		<span class="event">InstancesSent</span><br/><br/>
		<xsl:call-template name="BeginStoringInstancesType"/>
		<xsl:apply-templates select="RNode"/>
		<xsl:apply-templates select="InstanceActionDescription"/>
	</xsl:template>
	...
	<xsl:template match="DICOMInstancesUsed">
		<span class="event">DICOMInstancesUsed</span><br/><br/>
		<xsl:call-template name="InstancesActionType"/>
	</xsl:template>
	<!-- StudyDeleted -->
	...
	<xsl:template match="DicomQuery">
		<span class="event">DicomQuery</span><br/><br/>
	...
	<xsl:template match="Requestor">
		<b>Requestor:</b><br/><div class="level"><xsl:call-template name="RemoteNodeType"/></div>
	</xsl:template>
	<!-- SecurityAlert -->
	...
	<xsl:template match="UserAuthenticated">
		<span class="event">UserAuthenticated</span><br/><br/>
		<xsl:if test="LocalUsername"><b>LocalUsername: </b><xsl:value-of select="LocalUsername"/><br/></xsl:if>
		<xsl:apply-templates select="Action"/>
	</xsl:template>
	<!-- AuditLogUsed -->
	<xsl:template match="AuditLogUsed">
		<span class="event">AuditLogUsed</span><br/><br/>
		<xsl:apply-templates select="Usage"/>
		<xsl:apply-templates select="User"/>
	</xsl:template>
	<!-- NetworkEntry -->
	<xsl:template match="NetworkEntry">
		<span class="event">NetworkEntry</span><br/><br/>
	...
	<xsl:template match="TimeStamp"> <!-- make timestamp into a pretty link -->
        <xsl:variable name="timezone-found">
            <xsl:choose>
                <xsl:when test="contains(.,'z') or contains(.,'Z') or contains(substring-after(.,'T'),'-') or contains(.,'+')">true</xsl:when>
	...
	<xsl:template match="IP"><b>IP: </b><span class="ip"><xsl:value-of select="."/></span><br/></xsl:template>
	<xsl:template match="AET"><b>AET: </b><xsl:value-of select="."/><br/></xsl:template>
	<xsl:template match="Hname"><b>Hname: </b><xsl:value-of select="."/><br/></xsl:template>
	<xsl:template match="MachineAction"><b>MachineAction: </b><xsl:value-of select="."/><br/></xsl:template>
	<xsl:template match="ObjectAction"><b>ObjectAction: </b>
		<xsl:choose>
			<xsl:when test=". = 'Create'"><span style="font-style: italic"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:when test=". = 'Access'"><span style="font-style: italic"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:when test=". = 'Modify'"><span style="font-style: italic"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:when test=". = 'Delete'"><span style="font-style: italic"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:otherwise><span style=""> [Invalid or unspecified]</span></xsl:otherwise>
		</xsl:choose>
		<br/>
	</xsl:template>
	<xsl:template match="CUID"><b>CUID: </b><span class="id"><xsl:value-of select="."/></span><br/></xsl:template>
	<xsl:template match="SUID"><b>SUID: </b><span class="id"><xsl:value-of select="."/></span><br/></xsl:template>
	<xsl:template match="MPPSUID"><b>MPPSUID: </b><span class="id"><xsl:value-of select="."/></span><br/></xsl:template>
	<xsl:template match="Keys">
        <form name="frmviewdcm" action="arr-viewdcm.do" method="post">
            <input type="hidden" name="base64" value="{.}"/>
            <b>Keys: </b><input type="submit" name="submit" value="view..."/>
        </form>
    </xsl:template>
	<xsl:template match="AccessionNumber"><b>AccessionNumber: </b><span class="num"><xsl:value-of select="."/></span><br/></xsl:template>
	<xsl:template match="ApplicationAction"><b>ApplicationAction:</b>
		<xsl:choose>
			<xsl:when test=". = 'Start'"><span style="font-style: italic"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:when test=". = 'Stop'"><span style="font-style: italic"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:otherwise><span style=""> [Invalid or unspecified]</span></xsl:otherwise>
		</xsl:choose>
		<br/>
	</xsl:template>
	<xsl:template match="PatientID"><b>PatientID: </b><span class="id"><xsl:value-of select="."/></span><br/></xsl:template>
	<xsl:template match="PatientName"><b>PatientName: </b><xsl:value-of select="."/><br/></xsl:template>
	<xsl:template match="AlertType"><b>AlertType: </b><xsl:value-of select="."/><br/></xsl:template>
	<xsl:template match="Action"><b>Action:</b>
		<xsl:choose>
			<xsl:when test=". = 'Login'"><span style="color: green"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:when test=". = 'Logout'"><span style="color: black"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:when test=". = 'Failure'"><span style="color: red"> <xsl:value-of select="."/></span></xsl:when>
			<xsl:otherwise><span style=""> [Invalid or unspecified]</span></xsl:otherwise>
		</xsl:choose>
		<br/>
	</xsl:template>
    ...
    <xsl:template match="LocalPrinter"><b>LocalPrinter: </b><xsl:value-of select="."/><br/></xsl:template>
	
	<!-- Named templates are used for aggregate types -->
	<xsl:template name="MediaDescriptionType">
		<xsl:if test="MediaID"><b>MediaID: </b><xsl:value-of select="MediaID"/><br/></xsl:if>
		<xsl:if test="MediaType"><b>MediaType: </b><xsl:value-of select="MediaType"/><br/></xsl:if>
		<xsl:apply-templates select="SUID"/>
		<xsl:apply-templates select="Patient"/>
	...
	<xsl:template name="InstancesActionType"> <!-- renamed, called InstancesAction in xsd -->
		<xsl:apply-templates select="ObjectAction"/>
		<xsl:apply-templates select="AccessionNumber"/>
		<xsl:apply-templates select="SUID"/>
		<xsl:apply-templates select="Patient"/>
		<xsl:apply-templates select="User"/>
		<xsl:apply-templates select="CUID"/>
		<xsl:if test="NumberOfInstances"><b>NumberOfInstances: </b><xsl:value-of select="NumberOfInstances"/><br/></xsl:if>

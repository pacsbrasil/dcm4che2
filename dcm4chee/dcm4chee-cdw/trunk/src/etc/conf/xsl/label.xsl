			...
			<fo:page-sequence master-reference="page">
				<fo:flow flow-name="xsl-region-body">
					<fo:block-container position="absolute" top="5mm" left="35mm" right="85mm" bottom="20mm">
						<xsl:variable name="labeltext" select="dicomdir/attr[@tag='(2200,0002)']"/>
						<xsl:choose>
						<!-- if Label Text -->	
					...
					<xsl:if test="dicomdir/attr[@tag='(2200,0009)']='YES'">
					...
					<fo:block-container position="absolute" top="107mm" left="20mm" right="100mm" bottom="120mm">
						...
						<xsl:if test="$fsid!=''">
							<fo:block text-align="center" font-family="Helvetica" font-size="12pt">
								File-set ID: <xsl:value-of select="$fsid"/>
							</fo:block>
						</xsl:if>
						<!-- if several disks -->
						<xsl:if test="$size &gt; 1">
							<fo:block text-align="center" font-family="Helvetica" font-size="12pt">
								Disk #<xsl:value-of select="$seqno"/> of <xsl:value-of select="$size"/>
							</fo:block>
						</xsl:if>
					</fo:block-container>
				</fo:flow>
...
<xsl:template match="dicomdir/record" mode="list-patient">
...
<xsl:template name="list-requirements">
    <fo:block padding-bottom="2mm" font-family="Helvetica" font-size="9pt">System Requirements:</fo:block>
		<fo:list-block font-family="Helvetica" font-size="8pt">
			<fo:list-item>
				<fo:list-item-label><fo:block>Processor:</fo:block></fo:list-item-label>
				<fo:list-item-body start-indent="15mm"><fo:block>P3/500MHz</fo:block></fo:list-item-body>
			</fo:list-item>
			<fo:list-item>
				<fo:list-item-label><fo:block>Memory:</fo:block></fo:list-item-label>
				<fo:list-item-body start-indent="15mm"><fo:block>min 256MB</fo:block></fo:list-item-body>
			</fo:list-item>
			<fo:list-item>
				<fo:list-item-label><fo:block>OS:</fo:block></fo:list-item-label>
				<fo:list-item-body start-indent="15mm"><fo:block>WinNT/2k/XP/Me</fo:block></fo:list-item-body>
			</fo:list-item>
		</fo:list-block>
</xsl:template>				

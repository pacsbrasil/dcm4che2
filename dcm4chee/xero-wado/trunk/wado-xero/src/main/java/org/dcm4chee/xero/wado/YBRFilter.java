...
/** Fixes the YBR color Space Images, assuming that "raw" images never need to
 * be fixed as they are being requested already in the correct space.
 */
public class YBRFilter  implements Filter<WadoImage> {
    ...
    public WadoImage filter(FilterItem<WadoImage> filterItem, Map<String, Object> params) {
        ...
        if( bi==null ) return wi;
        ColorModel cm = bi.getColorModel();
        if( cm.getNumComponents()!=3 ) return wi;
        ColorSpace cs = cm.getColorSpace();
        if(! ( cs instanceof SimpleYBRColorSpace) ) return wi;

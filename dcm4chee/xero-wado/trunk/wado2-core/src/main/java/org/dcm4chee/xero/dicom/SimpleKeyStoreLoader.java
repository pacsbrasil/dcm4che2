...
/**
 * Basic Keystore selector that will choose a keystore based on the 
 * name of the file.
 * @author Andrew Cowan (andrew.cowan@agfa.com)
 */
public class SimpleKeyStoreLoader implements KeyStoreLoader
{
   // TODO: Support URLs as well so that resource extraction can be used for unit testing...
      ...
      throws GeneralSecurityException, IOException
   {
      if(fileName == null) 
         return null;
      if(password == null)
         throw new IllegalArgumentException("Password for keystore '"+fileName+"' is undefined.");
      
      KeyStore key = KeyStore.getInstance(toKeyStoreType(fileName));
      InputStream in = new FileInputStream(fileName);
      try
      {
         key.load(in, password.toCharArray());
      }
      finally
      {
         in.close();

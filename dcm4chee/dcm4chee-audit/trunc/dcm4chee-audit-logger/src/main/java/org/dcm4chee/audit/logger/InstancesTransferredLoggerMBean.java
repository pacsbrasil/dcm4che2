...
public class InstancesTransferredLoggerMBean 
    ...
    private void registerInstancesTransferredListener() 
            throws Exception {
        NotificationFilterSupport f = new NotificationFilterSupport();
        f.enableType(InstancesTransferred.class.getName());
        server.addNotificationListener(queryRetrieveSCPName,
                instancesTransferredListener, f , null);
    }

    private void unregisterInstancesTransferredListener() {
        try {
            server.removeNotificationListener(queryRetrieveSCPName, 
    ...
    private final NotificationListener instancesTransferredListener = 
            new NotificationListener() {

        public void handleNotification(Notification notif, Object handback) {
            try {
                InstancesTransferred transferred = 
                        (InstancesTransferred) notif.getUserData();
    ...
    private Study mkStudy(Map.Entry entry) {
        Study study = new Study((String) entry.getKey());
        Map cuids = (Map) entry.getValue();
        ...
        for (Iterator iter = cuids.entrySet().iterator(); iter.hasNext();) {
            ...
            ParticipantObjectDescription.SOPClass sopClass =
                    new ParticipantObjectDescription.SOPClass(
                        (String) el.getKey());
            sopClass.setNumberOfInstances(((Collection) el.getValue()).size());

...
public class ScannerCalibration {
	 ...
	 */
	public float[] readODs(File f) throws IOException {
		...
		try {
			...
			throw new IOException(
				"Error in line " + lineNo + " in " + f + ": " + e.getMessage());
		} finally {
			r.close();
		}
	}
	 ...
	 * @exception  IOException  Description of the Exception
	 */
	public void writeODs(File f, float[] ods) throws IOException {
		if (ods.length != 256) {
	...
	private final FilenameFilter JPG_FILENAME_FILTER = new FilenameFilter() {
	...
	public File getMostRecentScanFile(String scanDirName) {
		...
		Arrays.sort(scanFiles, new Comparator() {
			public int compare(Object o1, Object o2) {
				long diff =
					(((File) o2).lastModified() - ((File) o1).lastModified());
		...
		if (refODs == null || invRefPx == null) {
			...
			if (invRefPx.length != refODs.length)
				throw new CalibrationException(
					"Number of reference ODs ("
						+ refODs.length
						+ ") does not match samples from reference image ("
						+ invRefPx.length
						+ ")");

			ReferenceSample[] invRefPxTagged =
				new ReferenceSample[invRefPx.length];
	...
	private ImageReader findReader(File f) throws CalibrationException {
	 ...
	 * @exception  IOException           Description of the Exception
	 */
	public float[] analyse(File f) throws CalibrationException, IOException {
		if (log.isDebugEnabled()) {
			log.debug("Analyse Scan " + f);
		}
		...
		try {
			...
			int[] hline =
				(bi = r.read(0, rParam)).getRGB(0, 0, w, 1, null, 0, w);
			...
			if (log.isDebugEnabled()) {
				StringBuffer sb =
					new StringBuffer("detected Grayscale 255-pxval:");
	...
	private float[] sampleBoxPattern(
		int[] pixels,
		final int width,
		final int height,
		final int nx,
	...
	private int[] findBorder(int[] rgb) throws CalibrationException {
	...
	public static void main(String args[]) throws Exception {
		...
		try {
			...
			System.err.println(
				"probescan: i/o error - " + e.getMessage() + "\n");
			System.exit(1);
		}
	}

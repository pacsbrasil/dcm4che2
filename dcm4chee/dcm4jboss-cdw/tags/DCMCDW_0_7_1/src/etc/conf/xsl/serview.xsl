...
<xsl:template match="record[@type='SERIES']" mode="index">
	...
	<xsl:variable name="serdir" select="substring(record/attr[@tag='(0004,1500)'],7,27)"/>
	<redirect:write select="concat($serdir,'INDEX.HTM')">
		<html>
			...
			<body background="../../../BACKGRND.JPG" link="#FF0000" alink="#FF0000" vlink="#FF0000">
				<table width="100%">
					<tr>
						<td><a href="../../../HOME.HTM"><b>Home</b></a>&#160;&gt;&#160;<a href="../../INDEX.HTM"><b>Patient</b></a>&#160;&gt;&#160;<a href="../INDEX.HTM"><b>Study</b></a>&#160;&gt;&#160;<b>Series</b><br/><a href="../../../INDEX.HTM" target="_top">FRAMES</a>&#160;&#160;<a href="INDEX.HTM" target="_top">NO FRAMES</a></td>
						<td align="right">
				...
				<table border="0" cellspacing="1" cellpadding="1">
					<xsl:apply-templates select="record">
						<xsl:sort data-type="number" order="ascending" select="attr[@tag='(0020,0013)']"/>
					</xsl:apply-templates>
				</table>
			</body>
		</html>
...
<xsl:template match="record[@type='IMAGE']">
	<tr>
		<td>
			<b>Image</b>&#160;<xsl:value-of select="attr[@tag='(0020,0013)']"/>&#160;<xsl:value-of select="attr[@tag='(0008,0008)']"/>
			<table>
				<tr>
					<xsl:call-template name="frame">
						<xsl:with-param name="imgdir" select="substring(attr[@tag='(0004,1500)'],34)"/>
...
<xsl:template name="frame">
	...
	<td>
		<img src="{concat($imgdir,'/',$frame,'.JPG')}"/>
		<xsl:if test="$frames!=''">
...
<xsl:template match="record[@type='SR DOCUMENT']">
	...
	<tr>
		...
		<td>
			<xsl:value-of select="attr[@tag='(0040,A043)']/item/attr[@tag='(0008,0104)']"/>
		</td>
		<td><xsl:value-of select="attr[@tag='(0040,A491)']"/>/<xsl:value-of select="attr[@tag='(0040,A493)']"/></td>
	</tr>
</xsl:template>

...
public class XDSDocumentFileStorage implements XDSDocumentStorage {
	...
	public StoredDocument storeDocument(XDSDocument xdsDoc) throws XDSException {
		...
		try {
			...
			return digest == null ? null : new StoredDocument(xdsDoc.getDocumentUID(), 
					xdsDoc.getXdsDocWriter().size(),digest, f.getAbsolutePath());
		} catch ( Throwable x ) {
			log.error("Storage of document failed:"+xdsDoc.getDocumentUID(), x);
			throw new XDSException(XDSConstants.XDS_ERR_REPOSITORY_ERROR, 
					"Storage of document failed:"+xdsDoc.getDocumentUID(), x);
		} finally {
			try {

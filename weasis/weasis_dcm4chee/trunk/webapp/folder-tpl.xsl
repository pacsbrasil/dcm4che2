<?xml version="1.0" encoding="UTF-8"?>
	<!-- $Id$ -->
	<!--
		Enable/disable the patient folder operations to match the project
		requirements TODO: Remove project specific hardcoded values
	-->
		...
		<table class="folder_header" border="0" cellspacing="0"
				...
				<table class="folder_header" border="0" height="30"
					...
					<tr>
						<td class="folder_header">
							...
							<div title="&ListStudiesOfOnePatientBeginningWithTheMostRecentStudy;">
						...
						<td class="folder_header" align="center">
							<xsl:choose>
								<xsl:when test="total &lt; 0">&NoQueryPerformed;
								</xsl:when>
								<xsl:when test="total &lt; 1">&NoMatchingStudiesFound;
								</xsl:when>
								<xsl:otherwise>
									<xsl:if test="showWithoutStudies='false'">
										<xsl:text>&DisplayingStudies;
										</xsl:text>
									</xsl:if>
									<xsl:if test="showWithoutStudies='true'">
										<xsl:text>&DisplayingStudiesAndEmptyPatients;
										</xsl:text>
									</xsl:if>
									<b>
									...
									<xsl:if test="showWithoutStudies='false'">
										<xsl:text> &matchingStudies;
										</xsl:text>
									</xsl:if>
									<xsl:if test="showWithoutStudies='true'">
										<xsl:text> &matchingStudiesAndEmptyPatients;
										</xsl:text>
									</xsl:if>
								</xsl:otherwise>
							</xsl:choose>
						...
						<td class="folder_header" width="40">
							<input type="image" value="Search" name="filter" src="images/search.gif"
								border="0" title="&NewSearch;" />
						</td>
						<td class="folder_header" width="40">
						...
						<xsl:if test="$folder.edit='true'">
							<td class="folder_header" width="40">
								<a href="patientEdit.m?pk=-1">
									<img src="images/addpat.gif" alt="Add Patient" border="0"
										title="&AddNewPatient;" />
								</a>
							</td>
						</xsl:if>
						<xsl:if test="$folder.mergepat='true'">
							<td class="folder_header" width="40">
								<input type="image" value="Merge" name="merge" src="images/merge.gif"
									alt="merge" border="0" title="&MergeSelectedPatients;"
						...
						<xsl:if test="$folder.move='true'">
							<td class="folder_header" width="40">
								<input type="image" value="Move" name="move" src="images/move.gif"
									alt="move" border="0" onclick="return confirm('&MoveSelectedEntities;?')"
						...
						<xsl:if test="$folder.export_tf='true'">
							<td class="folder_header" width="40">
								<input type="image" value="Export" name="exportTF"
									src="images/export_tf.gif" alt="TF Export" border="0"
						...
						<xsl:if test="$folder.export_xds='true'">
							<td class="folder_header" width="40">
								<input type="image" value="xdsi" name="exportXDSI"
									src="images/export_xdsi.gif" alt="XDSI Export" border="0"
									title="&ExportSelectedEntitiesToXDSRepository;">
									<xsl:if test="total &lt;= 0">
										<xsl:attribute name="disabled">disabled</xsl:attribute>
									</xsl:if>
								</input>
						...
						<xsl:if test="$folder.delete='true'">
							<td class="folder_header" width="40">
								<input type="image" value="Del" name="del" src="images/trash.gif"
									alt="delete" border="0" title="&DeleteSelectedEntities;"
						...
						<xsl:if test="$folder.send='true'">
							<td class="folder_header" width="40">
								<input type="image" value="Send" name="send" src="images/send.gif"
									alt="send" border="0"
         ...
         + '&confirmSendSelectedEntitiesTo2;')">
						...
						<td class="folder_header" width="50">
							<select size="1" name="destination" title="&SendDestination;">
						...
						<td class="folder_header" width="5">
							<input type="checkbox" name="filterAET" value="true"
					...
					<tr>
						<td colspan="5">
							<xsl:if test="$folder.query_has_issuer='true'">
								<div title="&QueryHasIssuer;">
									<xsl:text>&QueryHasIssuer_text1;
									</xsl:text>
									<input type="checkbox" name="hideHasNoIssuerOfPID" value="true"
										...
										<xsl:if test="hideHasNoIssuerOfPID = 'true'">
											<xsl:attribute name="checked" />
										</xsl:if>
									</input>
									<xsl:text>&QueryHasIssuer_text2;
									</xsl:text>
									<input type="checkbox" name="hideHasIssuerOfPID" value="true"
										...
										<xsl:if test="hideHasIssuerOfPID = 'true'">
											<xsl:attribute name="checked" />
										</xsl:if>
									</input>
									<xsl:text>&QueryHasIssuer_text3;
									</xsl:text>
								</div>
							</xsl:if>
						</td>
				...
				<table class="folder_search" border="0" width="100%"
					...
					<tr>
						...
						<td>
							<input size="10" name="patientID" type="text" title="&formatPatientID;"
								value="{patientID}" />
						</td>
						<td></td>
						<xsl:choose>
							<xsl:when test="showStudyIUID='true'">
								<td class="label">&StudyIUID;:
									<div title="&AlternativeStudyUID;">
										<input name="alternativeStudyUID" type="hidden"
											value="{alternativeStudyUID}" />
										...
										<xsl:text>&AlternativeStudyUID_text;
										</xsl:text>
									</div>
								</td>
								<td>
							...
							<xsl:otherwise>
								...
								<xsl:choose>
									<xsl:when test="filterBirthdate='false'">
										...
										<td>
											<input size="10" name="studyDateRange" type="text"
												value="{studyDateRange}" title="&formatStudyDate;" />
									...
									<xsl:otherwise>
										...
										<td>
											<input size="10" name="birthdate" type="text" value="{birthdate}"
					...
					<font size="1">
						<xsl:choose>
							<xsl:when test="showStudyIUID='false'">
								<b>
									<xsl:text>&StudyDescription;
									</xsl:text>
								</b>
								<xsl:text>/</xsl:text>
								<a title="&ShowStudyIUID;" href="foldersubmit.m?showStudyIUID=true&amp;studyID=">
									&StudyIUID;
								</a>
							</xsl:when>
							<xsl:otherwise>
								<a title="&ShowStudyDescription;" href="foldersubmit.m?showStudyIUID=false&amp;studyUID=">
									&StudyDescription;
								</a>
								<xsl:text>/</xsl:text>
								<b>&StudyIUID;
					...
					<font size="1">
						<xsl:choose>
							<xsl:when test="showSeriesIUID='false'">
								...
								<xsl:text>/</xsl:text>
								<a title="&ShowSeriesIUID;" href="foldersubmit.m?showSeriesIUID=true">
									&SeriesIUID;
								</a>
							</xsl:when>
							<xsl:otherwise>
								<a title="&ShowSeriesDescription;" href="foldersubmit.m?showSeriesIUID=false">
		...
		<table class="patient_line" width="100%" cellpadding="0"
			...
			<td class="patient_mark" align="right" rowspan="{$rowspan}">
				<xsl:choose>
					<xsl:when test="showStudies='false'">
						<a title="&ShowStudies;" href="expandPat.m?patPk={pk}&amp;expand=true">
							<img src="images/plus.gif" border="0" alt="+" />
						</a>
					</xsl:when>
					<xsl:otherwise>
						<a title="&HideStudies;" href="expandPat.m?patPk={pk}&amp;expand=false">
			...
			<td title="&PatientID;">
				<strong>
					...
					<xsl:if
						test="/model/showIssuerOfPID = 'true' and issuerOfPatientID != ''">
						<xsl:text>^^^</xsl:text>
			...
			<td class="patient_mark" align="right">
				<xsl:if test="$folder.xds_consumer='true' and /model/XDSConsumer='true'">
					...
					<xsl:choose>
						<xsl:when test="showXDS='false'">
							<a title="&ShowXDSDocuments;" href="expandXDS.m?patPk={pk}&amp;expand=true">
								<img src="images/plus.gif" border="0" alt="+" />
							</a>
						</xsl:when>
						<xsl:otherwise>
							<a title="&HideXDSDocuments;" href="expandXDS.m?patPk={pk}&amp;expand=false">
								<img src="images/minus.gif" border="0" alt="-" />
							</a>
						</xsl:otherwise>
					</xsl:choose>
				...
				<xsl:if test="$folder.edit='true'">
					<a href="studyEdit.m?patPk={pk}&amp;studyPk=-1">
						<img src="images/add.gif" alt="icon" border="0" title="&AddNewStudy;" />
					</a>
					...
					<xsl:if test="$folder.study_permission='true'">
						<a href="studyPermission.m?patPk={pk}">
							<img src="images/permission.gif" alt="icon" border="0"
								title="&ShowStudyPermissionsForPatient;" />
						</a>
					</xsl:if>
				</xsl:if>
				<a href="inspectDicomHeader.m?patPk={pk}" target="dbAttrs">
					<img src="images/dbattrs.gif" alt="icon" border="0"
						title="&ShowPatientAttributesInDB;" />
				</a>
			...
			<td class="study_mark" align="right">
				<xsl:if test="/model/webViewer='true'">
					<xsl:choose>
						...
						<xsl:when test="modalitiesInStudy='KO'">
							<!--
								no webviewer action if study contains only KO !
							-->
						</xsl:when>
						<xsl:otherwise>
							<a href="/dcm4chee-webview/webviewer.jsp?studyUID={studyIUID}">
								<xsl:attribute name="onclick">
									...
									<xsl:value-of select="/model/webViewerWindowName" />
									<xsl:text>','/dcm4chee-webview/webviewer.jsp?studyUID=</xsl:text>
									<xsl:value-of select="studyIUID" />
									<xsl:text>')</xsl:text>
								</xsl:attribute>
								<img src="images/webview.gif" alt="icon" border="0"
									title="&ViewStudyInWebviewer;" />
							</a>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:if>
				<xsl:if test="$folder.edit='true'">
					<xsl:if test="$folder.add_worklist='true'">
						<a href="addWorklist.m?studyPk={pk}">
							<img src="images/worklist.gif" alt="icon" border="0"
								title="&AddWorklistItem;" />
						</a>
					</xsl:if>
					<a href="seriesEdit.m?patPk={../../pk}&amp;studyPk={pk}&amp;seriesPk=-1">
						<img src="images/add.gif" alt="icon" border="0" title="&AddNewSeries;" />
					</a>
					...
					<xsl:if test="$folder.study_permission='true'">
						<a href="studyPermission.m?studyIUID={studyIUID}&amp;patPk={../../pk}">
							<img src="images/permission.gif" alt="icon" border="0"
								title="Show Study Permissions" />
						</a>
					</xsl:if>
				</xsl:if>
				<a href="inspectDicomHeader.m?studyPk={pk}" target="studyAtrrs">
					<img src="images/dbattrs.gif" alt="icon" border="0"
						title="&ShowStudyAttributesInDB;" />
				</a>
		...
		<table class="series_line" width="100%" cellpadding="0"
			...
			<td class="series_mark" align="right">
					...
					<xsl:choose>
						...
						<xsl:when test="modality = 'KO'">
							<a href="/dcm4chee-webview/webviewer.jsp?seriesUID={seriesIUID}">
								...
								<img src="images/webview_ko.gif" alt="icon" border="0"
									title="&ViewKeyObjectsInWebviewer;" />
							</a>
						</xsl:when>
					</xsl:choose>
				</xsl:if>
				...
				<a href="inspectDicomHeader.m?seriesPk={pk}" target="dbAttrs">
					<img src="images/dbattrs.gif" alt="icon" border="0"
						title="&ShowSeriesAttributesInDB;" />
				</a>
				...
				<xsl:choose>
					<xsl:when test="availability='ONLINE' or availability='NEARLINE'">
						<a
							...
							target="imageview">
							<img src="images/image.gif" alt="icon" border="0" title="&ViewImage;" />
						</a>
						<a href="inspectDicomHeader.m?instancePk={pk}" target="dbAttrs">
							<img src="images/dbattrs.gif" alt="icon" border="0"
								title="&ShowInstanceAttributesInDB;" />
						</a>
						<a
							...
							target="_blank">
							<img src="images/attrs.gif" alt="icon" border="0"
								title="&ShowDICOMAttributes;" />
						</a>
						<a
							...
							<img src="images/save.gif" alt="icon" border="0"
								title="&DownloadDICOMObject;" />
						</a>
					</xsl:when>
					<xsl:otherwise>
						<img src="images/invalid.gif" alt="icon" border="0" title="&notOnline;" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td class="instance_mark" align="right">
					...
					<xsl:when test="availability='ONLINE'">
						<xsl:if test="/model/webViewer='true' and ../../modality='PR'">
							<a href="/dcm4chee-webview/webviewer.jsp?prUID={sopIUID}">
								...
								<img src="images/webview_pr.gif" alt="icon" border="0"
					...
					<td class="instance_mark" align="right">
						...
						<a
							...
							<img src="images/sr.gif" alt="icon" border="0"
								title="&ViewReportAsHTML;" />
						</a>
						<a href="xdsiExport.m?docUID={sopIUID}">
							<img src="images/xds.gif" alt="icon" border="0"
								title="&ExportPDFtoXDSRepository;" />
						</a>
						<a href="inspectDicomHeader.m?instancePk={pk}" target="dbAttrs">
							<img src="images/dbattrs.gif" alt="icon" border="0"
								title="&ShowInstanceAttributesInDB;" />
			...
			<td>
				<xsl:if test="position()=1 and ../../availability='ONLINE'">
					<a
						...
						target="_blank">
						<img src="images/attrs.gif" alt="icon" border="0"
							title="&ShowOriginalReceivedDICOMAttributes;" />
					</a>
					<a
...
<xsl:template name="xds_documents">
	<tr>
		<table class="xds_docs" width="100%">
			...
			<tr>
				<td class="xds_docs_nav" title="&XDSDocuments;" colspan="8">&XDSDocuments;:&#160;&#160;
					<a href="xdsQuery.m?queryType=findDocuments&amp;patPk={pk}">
						<img src="images/search.gif" alt="icon" border="0"
							title="&QueryForXDSDocuments;" />
					</a>
					<a href="xdsQuery.m?queryType=clearDocumentList&amp;patPk={pk}">
						<img src="images/loeschen.gif" alt="icon" border="0"
							title="&ClearListOfXDSDocuments;" />
					</a>
				</td>
			</tr>
			<tr>
			...
			<xsl:apply-templates select="XDSDocuments/item"
				mode="xds" />
		</table>
	</tr>
</xsl:template>
	...
	<tr>
				...
				<xsl:choose>
					<xsl:when test="mimeType='application/dicom'">
						<a href="showManifest.m?url={URL}&amp;documentID={id}" target="xdsManifest">
							<img src="images/image.gif" alt="icon" border="0"
								title="&OpenXDSIManifest;" />
						</a>
						<xsl:if test="/model/webViewer='true'">
		...
		<td>
			<xsl:if test="repositoryUID and not(repositoryUID='')">
				<a
					href="/dcm4chee-xdsb-webretrieve?docUID={docUid}&amp;repoUID={repositoryUID}&amp;homeID={homeCommunityID}"
					target="xdsdoc">
					<img src="images/xdsb_retr.gif" alt="icon" border="0"
						title="OpenDocViaXDS.b" />
				</a>
			</xsl:if>
		</td>
	</tr>
